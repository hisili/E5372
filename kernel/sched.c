...
/*2011-10-24 c54299 added*/


FUNCPTR g_pTaskSwitchTable [LINUX_MAX_TASK_SWITCH_RTNS + 1]={NULL};
	       ...
	       struct task_struct *next)
{
	...
	spin_release(&rq->lock.dep_map, 1, _THIS_IP_);
      ...
      /*2011-10-24 c54299 added begin*/
      /* do switch hooks */
      for (ix = 0; (ix < LINUX_MAX_TASK_SWITCH_RTNS) && (g_pTaskSwitchTable[ix] != NULL); ++ix)
      {
          (* g_pTaskSwitchTable[ix]) ((void *)prev, (void *)next);
      }
      /*2011-10-24 c54299 added end*/
...
need_resched_nonpreemptible:
	...
	if (likely(prev != next)) {
        ...
        if(sysctl_msa_switch)
        {
            msa_cputime_account(prev, rq->idle);
...
int task_switch_hook_add ( FUNCPTR switchHook)
{
    int status;

    status = hook_add_to_tail (switchHook, g_pTaskSwitchTable, LINUX_MAX_TASK_SWITCH_RTNS);
