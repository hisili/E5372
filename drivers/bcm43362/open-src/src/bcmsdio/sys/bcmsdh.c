...
bcmsdh_intr_enable(void *sdh)
{
    ...
    //sdioh_enable_func_intr();//w00176398   removed by ghentlai@broadcom.com
...
bcmsdh_intr_disable(void *sdh)
{
    ...
    //sdioh_disable_func_intr();//w00176398 removed by ghentlai@broadcom.com
	return (SDIOH_API_SUCCESS(status) ? 0 : BCME_ERROR);
#else
	return 0;
#endif
...
int
bcmsdh_recv_skbbuf(void *sdh, uint32 addr, uint fn, uint flags,
                uint8 *buf, uint nbytes, void *pkt,
                bcmsdh_cmplt_fn_t complete_fn, void *handle)
{
	bcmsdh_info_t *bcmsdh = (bcmsdh_info_t *)sdh;
	...
	status = sdioh_request_skb(bcmsdh->sdioh, SDIOH_DATA_DMA, incr_fix,
	                              SDIOH_READ, fn, addr, width, nbytes, buf, pkt);
...
int
bcmsdh_send_skbbuf(void *sdh, uint32 addr, uint fn, uint flags,
                uint8 *buf, uint nbytes, void *pkt,
                bcmsdh_cmplt_fn_t complete_fn, void *handle)
{
	bcmsdh_info_t *bcmsdh = (bcmsdh_info_t *)sdh;
