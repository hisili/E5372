...
int
dhd_set_hostwake(void *pat,bool bset);
...
static void
dhd_dbus_recv_pkt(void *handle, void *pkt, int len) /* fuxiaowei - add len */
{
	dhd_info_t *dhd = (dhd_info_t *)handle;
	int ifidx = 0;
	...
	if (dhd_prot_hdrpull(&dhd->pub, &ifidx, pkt) != 0) {
	...
	/* fuxiaowei - set the actual len of skb */
	PKTSETLEN(dhd->pub.osh, pkt, len - 4);
	/* fuxiaowei */
...
dhd_dbus_recv_buf(void *handle, uint8 *buf, int len)
{
	...
	dhd_dbus_recv_pkt(dhd, pkt, len); /* fuxiaowei - add len */
}
...
{    
	...
	if (dhdp->txoff)
	{
        ...
        //printk("%s: 100: txoff tx ret = %d\n", __FUNCTION__, dhdp->txoff);
...
dhd_op_if(dhd_if_t *ifp)
{
	...
	case DHD_IF_CHANGE:
	...
	#if defined(DHD_UPDATE_INTF_MAC)
		{
			...
			{		
				DHD_EVENT(("Get mac address %s got %s idx %d\n", ifp->name,buf,ifp->idx));
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	/* added by wangweichao , colletct stats according to intf */
	...
	#if 0
	{
		...
		uchar *info = (uchar *)skb->data;
		int j = 0;
		
		WIFI_TRACE(("%s: 100: tx skb len: %d\n",__FUNCTION__, skb->len));
		
		WIFI_TRACE(("%s: 180: tx skb content: \n",__FUNCTION__));
		for(j = 0; j < 32; j++)
		{
			WIFI_TRACE(("%02x ", info[j]));
		}
	...
	if (skb_headroom(skb) < dhd->pub.hdrlen + htsfdlystat_sz +USB_ADDR_ALIGN) {
		...
		#if 0
		if ( ISALIGNED(skb2->data,USB_ADDR_ALIGN) == 0)
			{
			int pdata;
			DHD_DATA(("Reloacated addr %p not aligned  \n",skb2->data));
			pdata=(unsigned int)skb2->data;
			skb2->data=(char *)ROUNDUP(pdata,USB_ADDR_ALIGN);
			DHD_DATA(("Reloacated a aligned addre %p	\n",skb2->data));
		}
		#endif
		dev_kfree_skb(skb);
		if ((skb = skb2) == NULL) {
	...
	#if 0
	{
		...
		WIFI_TRACE(("%s: 210: tx skb len: %d\n",__FUNCTION__, skb->len));
		
		WIFI_TRACE(("%s: 220: tx skb content: \n",__FUNCTION__));
		for(j = 0; j < 32; j++)
		{
			WIFI_TRACE(("%02x ", info[j]));
		}
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		/* fuxiaowei - the max size of skb packet is 1522, if size > 1522, drop it. */
		if (skb->len > (net->mtu + net->hard_header_len + dhd->pub.hdrlen)) {
...
//#if 1	//#ifdef DHD_CHANGE_MAC
		...
		if (skb->pkt_type == PACKET_MULTICAST) {
			...
			/*add by wangweichao, to colletc the stats according intf */
			ifp->stats.multicast++;
		}
...
dhd_ioctl_entry(struct net_device *net, struct ifreq *ifr, int cmd)
{
	...
	ifidx = dhd_net2idx(dhd, net);
	DHD_CTL(("%s:100: ifr_name %sifidx %d, cmd 0x%04x\n", __FUNCTION__, ifr->ifr_name, ifidx, cmd));
	...
	if (ioc.buf) {
		...
		*/
		...
		/* BEGIN: fuxiaowei 
				  for tx restart timer test.
				  this may affect wl commands, so don't open it if you don't need it.
		*/
		if (strstr(ioc.buf, "tx_restart"))
		{
    ...
    if(ioc.buf != NULL)
    {
        DHD_CTL(("%s:900: ioc.buf = %s\n", __FUNCTION__, ioc.buf));
    }
    else
    {
...
static int set_mac_add1_or2(struct ether_addr *pmacaddr)/*lint !e528*/
{
    ...
    sprint_len = snprintf(mac_addr_buf, sizeof(mac_addr_buf), "%02x:%02x:%02x:%02x:%02x:%02x", 
        pmacaddr->octet[0], pmacaddr->octet[1], pmacaddr->octet[2], 
        pmacaddr->octet[3], pmacaddr->octet[4], pmacaddr->octet[5]);/*数字0.1.2.3.4.5分别是mac地址的位数*/
    if (0 > sprint_len)
    {
...
dhd_attach(osl_t *osh, struct dhd_bus *bus, uint bus_hdrlen)
{
	...
	g_wifi_dev = net;
    WIFI_TRACE(("%s: 1100:net name  %s\n", __FUNCTION__, g_wifi_dev->name));
    WIFI_TRACE(("%s: 1200:dhd addr %p, dbus state : %d\n", __FUNCTION__, dhd, dhd->pub.busstate));
...
{
	int ret = 0;
    int gpioh= bset==TRUE?1:0;
	char iovbuf[WL_EVENTING_MASK_LEN + 12];	/*  Room for "event_msgs" + '\0' + bitvec  */
    dhd_pub_t * dhd= (dhd_pub_t *)pat;

    bcm_mkiovar("bus:hostwake", (char *)&gpioh, 4, iovbuf, sizeof(iovbuf));
	ret = dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
    DHD_TRACE((" set host wake is %d,%d \n",ret,gpioh));
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
...
#endif /* defined(OEM_ANDROID) && !defined(AP) && defined(WL_CFG80211) */
    ...
    {
        int gpioval=0;
        bcm_mkiovar("bus:hostwake", (char *)&gpioval, 4, iovbuf, sizeof(iovbuf));
        dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);
    }
#endif
...
dhd_dbus_probe_cb(void *arg, const char *desc, uint32 bustype, uint32 hdrlen)
{
	...
	pub->dbus = dbus_attach(osh, pub->rxsz, DHD_NRXQ, DHD_NTXQ,
		pub->info, &dhd_dbus_cbs, NULL);
	if (pub->dbus) {
...
#if defined(OEM_ANDROID) && defined(CONFIG_WIFI_CONTROL_FUNC)
	if (wl_android_wifictrl_func_add() < 0)
		goto fail_1;
#endif /* #if defined(OEM_ANDROID) && defined(CONFIG_WIFI_CONTROL_FUNC) */
...
#endif /* defined(OEM_ANDROID) && defined(CONFIG_WIFI_CONTROL_FUNC) */
...
int force_set_netif_queue(int start_falg, int ifidx)
{
    ...
    {        
        WIFI_TRACE(("%s: 300: start_falg = %d, ifidx = %d, invalid\n", __FUNCTION__, start_falg, ifidx));
    }
...
#if 0
int print_txoff_drop(int ifidx)
{

    WIFI_DETAIL(("%s: 100: txoff = %d\n", __FUNCTION__, g_dbug_info->txoff));
    WIFI_DETAIL(("%s: 200: txoff_drop_cnt = %u\n", __FUNCTION__, v7_dbus_ranger.txoff_drop_cnt));
    WIFI_DETAIL(("%s: 300: netif_queue_stopped = %d\n", __FUNCTION__, netif_queue_stopped(v7_dbus_ranger.dhd->iflist[ifidx]->net)));
