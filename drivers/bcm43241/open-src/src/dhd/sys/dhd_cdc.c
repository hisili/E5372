 ...
 *
 * $Id: dhd_cdc.c 348330 2012-08-01 14:27:31Z $
 *
...
dhd_wlfc_hanger_create(osl_t *osh, int max_items)
{
	...
	WLFC_DBGMESG(("%s wlfc size:%d \n",__FUNCTION__,WLFC_HANGER_SIZE(max_items)));
...
#endif
#ifdef BCMDBUS
		rc = dhd_dbus_txdata((dhd_pub_t *)ctx->dhdp, p);
#else
...
_dhd_wlfc_enque_suppressed(athost_wl_status_info_t* ctx, int prec, void* p)
{
	...
	*/
	...
	WLFC_DBGMESG(("%s enqu %d,pkt:%p,entry:%p,len:%d\n",__FUNCTION__,prec,p,entry,pktq_mlen(&entry->psq,((prec << 1) + 1))));
...
_dhd_wlfc_deque_sendq(athost_wl_status_info_t* ctx, int prec)
{
	...
	if (p != NULL) {
		...
		while ((p != NULL)) {
			...
			*/
			_dhd_wlfc_traffic_pending_check(ctx, entry, prec);
			WLFC_DBGMESG(("Move packet %p from %d to psq %d\n",p,prec,p,prec,entry->psq.len));
			
			p = pktq_pdeq(&ctx->SENDQ, prec);
			if (p == NULL)
				break;

			WLFC_DBGMESG(("Still find %p in this que\n",p,prec));
	...
	ewlfc_mac_entry_action_t action, uint8 ifid, uint8 iftype, uint8* ea)
{
	...
	WLFC_DBGMESG(("%s action:%d,ifid:%d,type:%d,handle:%d\n",__FUNCTION__,action,ifid,iftype,entry->mac_handle));
	...
	ewlfc_mac_entry_action_t action, uint8 ifid, uint8 iftype, uint8* ea)
{
	...
	WLFC_DBGMESG(("%s interface %d,type:%d\n",__FUNCTION__,ifid,iftype));
...
dhd_wlfc_FIFOcreditmap_update(void* state, uint8* credits)
{
	...
	WLFC_DBGMESG(("%s  FIFO credit:%d-%d-%d-%d-%d\n",__FUNCTION__,credits[0],credits[1],credits[2],credits[3],credits[4]));
...
dhd_wlfc_enque_sendq(void* state, int prec, void* p)
{
	...
	DHD_DATA(("%s sendding %d with %p cur %d,len:%d\n",__FUNCTION__,prec,p,ctx->SENDQ.len,PKTLEN(ctx->osh,p)));
	if ((state == NULL) ||
	...
	for (ac = AC_COUNT; ac >= 0; ac--) {
		...
		do {
			...
			if (commit_info.p == NULL)
				...
				ASSERT(ac == AC_COUNT);
				WLFC_DBGMESG(("Oh got multi pkt %p in ac %d\n",commit_info.p,ac));
...
dhd_wlfc_txstatus_update(dhd_pub_t *dhd, uint8* pkt_info)
{
	...
	if (ret != BCME_OK) {
		/* do something */
		DHD_ERROR(("Handle pkt update failed %d\n",ret));
		return ret;
	}

	WLFC_DBGMESG(("%s status:%d,slot:%d,remove:%d,pkt:%p,gen:%d,docheck:%d \n",__FUNCTION__,status_flag,WLFC_PKTID_HSLOT_GET(status),remove_from_hanger,pktbuf,WLFC_PKTID_GEN(status),\
		pktbuf?DHD_PKTTAG_CREDITCHECK(PKTTAG(pktbuf)):0x999));
...
dhd_wlfc_fifocreditback_indicate(dhd_pub_t *dhd, uint8* credits)
{
	...
	WLFC_DBGMESG(("%s cre:%d-%d-%d-%d-%d-%d \n",__FUNCTION__,credits[0],credits[1],credits[2],credits[3],\
		credits[4],credits[5]));
...
dhd_wlfc_dbg_senum_check(dhd_pub_t *dhd, uint8 *value)
{
	...
	WLFC_DBGMESG(("RXPKT: SEQ: %d, timestamp %d\n", value[1], timestamp));
	return BCME_OK;
}
...
dhd_wlfc_psmode_update(dhd_pub_t *dhd, uint8* value, uint8 type)
{
	...
	WLFC_DBGMESG(("%s mh:%d,desc:%p,t:%d,oldpsmode:%d\n",__FUNCTION__,mac_handle,desc,type,desc->state));
...
dhd_wlfc_interface_update(dhd_pub_t *dhd, uint8* value, uint8 type)
{
	...
	WLFC_DBGMESG(("%s ifid:%d,typ:%d\n",__FUNCTION__,if_id,type));
...
dhd_wlfc_credit_request(dhd_pub_t *dhd, uint8* value)
{
	...
	credit = value[0];
	desc = &table[WLFC_MAC_DESC_GET_LOOKUP_INDEX(mac_handle)];
	WLFC_DBGMESG(("%s mh:%x,cre:%d,desc:%p,req:%d,map:%x\n",__FUNCTION__,mac_handle,credit,desc,desc->requested_credit,value[2]));
...
dhd_wlfc_packet_request(dhd_pub_t *dhd, uint8* value)
{
	...
	desc = &table[WLFC_MAC_DESC_GET_LOOKUP_INDEX(mac_handle)];
	WLFC_DBGMESG(("%s mh:%x,pkt:%d,desc:%p,req:%d,map:%x\n",__FUNCTION__,mac_handle,packet_count,desc,desc->requested_packet,value[2]));
	...
	uint *reorder_info_len)
{
	...
	DHD_DATA(("%s: p:%p,tlv:%d,\n", __FUNCTION__,pktbuf,remainder));

	if (remainder) {
		while ((processed < (WLFC_MAX_PENDING_DATALEN * 2)) && (remainder > 0)) {
			type = tmpbuf[processed];
			DHD_DATA(("-> prod:%d,typ:%d,rem:%d,len:%d\n", processed,type,remainder,tmpbuf[processed + 1]));
...
dhd_wlfc_init(dhd_pub_t *dhd)
{
	...
	*/
	...
	if (dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0) < 0) {
		...
		*/
		DHD_ERROR(("dhd_wlfc_init(): successfully %s bdcv2 tlv signaling, %x\n",
...
dhd_wlfc_enable(dhd_pub_t *dhd)
{
	...
	WLFC_DBGMESG(("%s  going to enable WLFC\n",__FUNCTION__));
...
/*Linux服务器在作为FTP服务器时，会把发出的数据包的优先级设置得比较小（<3），从而影响数传
在这里，博通台湾工程师把小于3优先级的包都提高到了3*/
#define RAISE_BK_PRIO 1

/*按照协议，HSIC在两个512整数倍报文之间都要发一个空包，但是我们的HSIC协议没有这样做，
导致，FW接收到这样两个报文时，会认为是一个报文，出现了很大的聚合包，而解析不了。
而我们这里的解决方式是：把所有512字节的报文加上4个字节，使得不能成为512整数，减少这种情况。
如：使用小MAC本或者IPAD从单板上下载PDF使用说明时，很容易出现下载不了的情况*/
...
dhd_prot_hdrpush(dhd_pub_t *dhd, int ifidx, void *PKTBUF)
{
...
#ifdef USB_NON_ZERO_PATCH
    uint16 final_pkt_len = 0;
#endif
#endif /* BDC */
    ...
    /* BRCM: Add 4 bytes to avoid USB doing glom */
#ifdef USB_NON_ZERO_PATCH
    pad_len = (unsigned int)PKTDATA(dhd->osh, PKTBUF) % USB_ADDR_ALIGN; 
    final_pkt_len = PKTLEN(dhd->osh, pktbuf) + pad_len + BDC_HEADER_LEN;
    if((final_pkt_len % 512) == 0){
        printk("512 bytes packet got\n");
        pad_len += 4;
    }
#endif 
	PKTPUSH(dhd->osh, PKTBUF, BDC_HEADER_LEN + pad_len);
...
//add to improve linux server throughput
#ifdef RAISE_BK_PRIO
    if (likely(h->priority < 3) || unlikely(6 == h->priority) || unlikely(7 == h->priority)) {
        h->priority = 3;
    }
#endif
	h->flags2 = 0;
...
#endif
#ifdef PROP_TXSTATUS
DHD_DATA(("%s: Enter,if:%d,pkt:%p,buflen:%d,dir:%d,plen:%d\n", __FUNCTION__,*ifidx,pktbuf,reorder_info_len?*reorder_info_len:0,DHD_PKTTAG_PKTDIR(PKTTAG(pktbuf)),PKTLEN(dhd->osh, pktbuf) ));
#endif
...
#ifdef BCMDBUS
		dhd_wlfc_commit_packets(dhd->wlfc_state, (f_commitpkt_t)dhd_dbus_txdata,
			(void *)dhd);
#else
...
dhd_prot_init(dhd_pub_t *dhd)
{
	...
	if (ret < 0)
    {
        DHD_TRACE(("%s: 150: fail to get cur_etheraddr", __FUNCTION__));
		goto done;
    }
    {
        int i = 0;
        DHD_TRACE(("%s: 170: mac from broadcom nvram: ", __FUNCTION__));
        for(i = 0; i < 6; i++)
        {
            DHD_TRACE(("%02x:", buf[i]));
        }
    ...
    dhd_custom_get_mac_address(e5_nv_mac);

    {
        int i = 0;
        DHD_TRACE(("%s: 170: mac e5_nv_mac: ", __FUNCTION__));
        for(i = 0; i < 6; i++)
        {
            DHD_TRACE(("%02x:", e5_nv_mac[i]));
        }
        DHD_TRACE(("\n"));
    }

    memset(buf, 0, sizeof(buf));
    bcm_mkiovar("cur_etheraddr", (void *)&e5_nv_mac, ETHER_ADDR_LEN, buf, sizeof(buf));
	...
	memcpy(dhd->mac.octet, e5_nv_mac, ETHER_ADDR_LEN);
	...
	if (ret < 0)
    {		
        DHD_TRACE(("%s: 300: fail to get evice rev info \n", __FUNCTION__));
		goto done;
    }
    DHD_TRACE(("%s: 310: end to get evice rev info \n", __FUNCTION__));
