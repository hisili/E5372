...
extern struct sk_buff *dev_alloc_skb_exp(unsigned int len);
    ...
    struct sk_buff * temSkbBuf = NULL;
    DHD_ARPOE(("%s len:%d\n",__FUNCTION__,len));
	ASSERT(len<=1536);
    temSkbBuf = dev_alloc_skb_exp(len);
    if(temSkbBuf != NULL)
    {
        return temSkbBuf;
    }
    ...
    {
        DHD_ARPOE(("%s TTF Memory No Ready,Using System Skb!!! len:%d\n",__FUNCTION__,len));
...
#else //w00176398 add for using ttf skb
    #if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25))
	gfp_t flags = (in_atomic()) ? GFP_ATOMIC : GFP_KERNEL;
...
struct sk_buff *osl_alloc_skb_orig(unsigned int len)
{
    #if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 25))
	gfp_t flags = (in_atomic()) ? GFP_ATOMIC : GFP_KERNEL;
...
#if defined(BALONG_PLATFORM)
	if (send == TRUE) {
		skb = osl_alloc_skb_orig(len);
	}
	else
#endif
		skb = osl_alloc_skb(len);
...
osl_pktget_static(osl_t *osh, uint len)
{
	...
	if (len > (PAGE_SIZE*2))
	{
		printf("Do we really need this big skb??\n");
#if defined(BALONG_PLATFORM)
		return osl_pktget(osh, len, TRUE);
#else
