...
dhd_prot_hdrpush(dhd_pub_t *dhd, int ifidx, void *pktbuf)
{
	...
	h->dataOffset = 0;
#endif /* BDC */
    DHD_TIMER(("%s skb:0x%x head:0x%x index:%d\n",__func__,(unsigned int)pktbuf,(unsigned int)h,ifidx));
...
void set_ifidx_offset(int val)
{
    printk("ifidx:%d new ifidx:%d\n",ifidxOffset,val);
    ifidxOffset = val;
}

void set_ifidx_offset1(int val)
{
    printk("ifidx:%d new ifidx:%d\n",ifidxOffset,val);
    ifidxOffset1 = val;
}
...
int
dhd_prot_ifidx_no_offset(dhd_pub_t *dhd, int *ifidx, void *pktbuf)
{
#ifdef BDC
	struct bdc_header *h;
#endif
    ...
    DHD_TIMER(("%s skb:0x%x\n",__func__,(unsigned int)pktbuf));
    //SDPCM_FRAMETAG_LEN
	h = (struct bdc_header *)(PKTDATA(dhd->osh, pktbuf)+ ifidxOffset);
...
int
dhd_prot_ifidx(dhd_pub_t *dhd, int *ifidx, void *pktbuf)
{
#ifdef BDC
	struct bdc_header *h;
#endif
    ...
    DHD_TIMER(("%s skb:0x%x\n",__func__,(unsigned int)pktbuf));
    //SDPCM_FRAMETAG_LEN
	h = (struct bdc_header *)(PKTDATA(dhd->osh, pktbuf)+ ifidxOffset1);
