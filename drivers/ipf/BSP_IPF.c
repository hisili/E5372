...
BSP_S32 BSP_IPF_SetFilter(IPF_CHANNEL_TYPE_E eChnType, IPF_FILTER_CONFIG_S *pstFilterInfo, BSP_U32 u32FilterNum)
{
	...
	if(!g_u32IpfStartRestore) /* 低功耗恢复流程不需要保存*/
	{
		if(u32FilterNum > 0)
		{		
...
/*****************************************************************************
* 函 数 名     : BSP_IPF_RegisterDlFreeMemCb
*
* 功能描述  : 该接口用于注册下行释放内存回调函数
*                           
* 输入参数  : BSP_IPF_DlFreeMemCb pFnDlFreeMem
*
* 输出参数  : 无
* 返 回 值      : 无
*  
