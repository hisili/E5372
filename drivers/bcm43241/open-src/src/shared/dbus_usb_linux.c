 ...
 *
 * $Id: dbus_usb_linux.c 345150 2012-07-16 21:43:26Z $
 */
/*****************************************************************************************
...
#ifdef DBUS_LINUX_RXDPC
usbos_info_t *usbos_info_work = NULL;
struct work_struct ads_ul_work_dpc;
extern struct workqueue_struct *lte_data_process;
void dbus_usbos_recv_dpc_work();
...
#ifdef BCMUSBDEV_COMPOSITE
static int dbus_usbos_intf_num(struct usb_device *usb, int ifnum);
#endif /* BCMUSBDEV_COMPOSITE */
...
void dbus_usbos_recv_dpc_work()
{
    usbos_info_t *usbos_info = usbos_info_work;
...
dbus_usbos_recv_urb_submit(usbos_info_t *usbos_info, dbus_irb_rx_t *rxirb, uint32 ep_idx)
{
    ...
    PKTRESERVE(usbos_info->pub->osh, req->pkt, NET_IP_ALIGN);
	rxirb->buf = NULL;
...
dbus_usbos_recv_complete_handle(urb_req_t *req, int len, int status)
{
    ...
    queue_work(lte_data_process, &ads_ul_work_dpc);
#else
...
fail:
    ...
    //PKTSETLEN(usbos_info->pub->osh,rxirb->pkt,urb->actual_length);
	req->pkt = NULL;
#endif /* BCM_RPC_NOCOPY || BCM_RPC_RXNOCOPY */
...
#endif /* KERNEL26 */
{
	...
	wlan_if = dbus_usbos_intf_num(usb, wlan_if);
#endif /* BCMUSBDEV_COMPOSITE */
...
#ifdef BCMUSBDEV_COMPOSITE
	if (IFDESC(usb, wlan_if).bInterfaceNumber) {
#else
...
#ifdef BCMUSBDEV_COMPOSITE
		wlan_if = dbus_usbos_intf_num(usbos_info->usb, wlan_if);
...
#ifdef DBUS_LINUX_RXDPC
    INIT_WORK(&ads_ul_work_dpc, dbus_usbos_recv_dpc_work) ;
#endif
#ifndef DBUS_LINUX_RXDPC
 ...
 * scan the device struct for the required interface
 */
#ifdef BCMUSBDEV_COMPOSITE
static int
dbus_usbos_intf_num(struct usb_device *usb, int ifnum)
{
	int i, int_num = 0;

	for (i = 0; i < USB_COMPIF_MAX; i++) {
		if (IFDESC(usb, i).bInterfaceNumber == ifnum) {
			int_num = i;
			break;
		}
	}
