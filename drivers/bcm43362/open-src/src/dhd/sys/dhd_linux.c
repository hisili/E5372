...
// for 43362a0/43362a2 chip identify
...
extern STA_FLUX g_sta_flux;
//end

//y00167654 add change wifi netdev name
#define BALONG_WIFI_NET_DEV_NAME     "WiFi0"    //wifi default network device name
//end
...
/* Watchdog interval */
uint dhd_watchdog_ms = 10; // ghentlai@boardcom.com, default is 50ms
module_param(dhd_watchdog_ms, uint, 0);

#if defined(DHD_DEBUG)
/* Console poll interval */
uint dhd_console_ms = 250; // ghentlai@boardcom.com for turn-on polling mode
...
//w00176398 2011-12-22 add for record last rx/tx packets time
void getLastNetTime()
{
    printk("last Net Time:%d\n",g_sta_flux.last_net_time);
}
...
// comment by guanghua, ghentlai@broadcom.com
// if configure dhd_idletime = -1, means dhd is working under polling mode;
...
dhd_start_xmit(struct sk_buff *skb, struct net_device *net)
{
	...
	// TODO, make a copy for workaroud TTF issue
	#if defined (BOARD_ASIC_BIGPACK) && defined (VERSION_V3R2)
	#if 0 //w00176398
	if (DEFAULT_DEFINED != skb->private_mem.enType) 
	{
		struct sk_buff * newskb = NULL;

		newskb = PKTGET(dhd->pub.osh, PKTLEN(dhd->pub.osh, skb), TRUE);
		if (newskb == NULL)
			return -ENOMEM;

		memcpy(PKTDATA(dhd->pub.osh, newskb), PKTDATA(dhd->pub.osh, skb), PKTLEN(dhd->pub.osh, skb));
		PKTSETLEN(dhd->pub.osh, newskb, PKTLEN(dhd->pub.osh, skb));
		PKTFREE(dhd->pub.osh, skb,true);
		newskb->private_mem.enType = DEFAULT_DEFINED;
		skb = newskb;
	}
	#endif
    #endif
	...
	if (ret)
		dhd->pub.dstats[ifidx].tx_dropped++;
	else
		dhd->pub.dstats[ifidx].tx_packets++;
#else
	if (ret)
...
dhd_rx_frame(dhd_pub_t *dhdp, int ifidx, void *pktbuf, int numpkt, uint8 chan)
{
	...
	for (i = 0; pktbuf && i < numpkt; i++, pktbuf = pnext) {
		...
		if(WIFI_RX_FLOW_IS_SET())
		{

			DHD_INFO(("%s: flow control is set, drop packet\n",
			__FUNCTION__));
			PKTFREE(dhdp->osh, pktbuf, TRUE);
			continue;            
		}
...
#if defined(BALONG_PLATFORM)
			dhd->pub.dstats[ifidx].multicast++;
#endif
...
#if defined(BALONG_PLATFORM)
	if (ntoh16(skb->protocol) != ETHER_TYPE_BRCM) {
		dhdp->dstats[ifidx].rx_bytes += skb->len;
		dhdp->dstats[ifidx].rx_packets++; /* Local count */
		if(IS_STA_PACKAGE(ifidx))
		{
			//WIFI_STA_SPIN_LOCK(g_sta_flux.flux_spinlock);
			SET_WIFI_STA_LAST_NET_TIME(jiffies);
...
dhd_get_stats(struct net_device *net)
{
    ...
    DHD_TIMER(("%s: ifidx:%d tx_bytes:%d\n", __FUNCTION__,ifidx,dhd->pub.dstats[ifidx].tx_bytes));
#if defined(BALONG_PLATFORM)

	/* Copy dongle stats to net device stats */
	ifp->stats.rx_packets = dhd->pub.dstats[ifidx].rx_packets;
	ifp->stats.tx_packets = dhd->pub.dstats[ifidx].tx_packets;
	ifp->stats.rx_bytes = dhd->pub.dstats[ifidx].rx_bytes;
	ifp->stats.tx_bytes = dhd->pub.dstats[ifidx].tx_bytes;
	ifp->stats.rx_errors = dhd->pub.dstats[ifidx].rx_errors;
	ifp->stats.tx_errors = dhd->pub.dstats[ifidx].tx_errors;
	ifp->stats.rx_dropped = dhd->pub.dstats[ifidx].rx_dropped;
	ifp->stats.tx_dropped = dhd->pub.dstats[ifidx].tx_dropped;
	ifp->stats.multicast = dhd->pub.dstats[ifidx].multicast;
#else
	if (dhd->pub.up) {
#ifdef DHD_DEBUG
		if (module_remove) {
...
#else
        strcpy(net->name, BALONG_WIFI_NET_DEV_NAME);
#endif
...
dhd_bus_start(dhd_pub_t *dhdp)
{
	...
	else if (dhd->pub.busstate == DHD_BUS_DOWN){
		if ((fw_path == NULL) && (fw_path[0] == '\0'))
			printf("fw_path is NULL\n");
		if ((nv_path == NULL) && (nv_path[0] == '\0'))
			printf("nv_path is NULL\n");
	}
	...
	dhd->pub.tickcnt = 0;
       if (dhd_poll == TRUE) {
		DHD_ERROR(("%s: ===========enabling Polling mode[poll-interval%u]=============\n", __FUNCTION__, dhd_watchdog_ms));
		DHD_EVENT(("%s: calling dhd_os_wd_timer for starting polling polling_interval=%u", __FUNCTION__, dhd_watchdog_ms));
...
#if defined(OOB_INTR_ONLY)
#if defined(HW_OOB)
	DHD_ERROR(("%s: ===========enabling HW_OOB=============\n", __FUNCTION__));
#elif defined(FORCE_SW_OOB_LEVEL)
	DHD_ERROR(("%s: ===========enabling SW_OOB with level trigger=============\n", __FUNCTION__));
#else
	DHD_ERROR(("%s: ===========enabling SW_OOB=============\n", __FUNCTION__));
#endif /* HW_OOB */
...
/* enable dongle roaming event */
	...
	DHD_EVENT(("%s Bus is ready, do any protocol initialization\n", __func__));
...
dhd_net_attach(dhd_pub_t *dhdp, int ifidx)
{
	...
	if (ifidx == 0) {
		...
		//add by ghentlai@broadcom.com to up iface[0]'s mac-address as configured
		if (dhd->pub.mac.octet[1] != 0 && dhd->pub.mac.octet[3] != 0)
...
#ifdef BALONG_WIFI_TRACE
        /*uninitialization wifi trace thread*/
        wifi_trace_uninit();
#endif
...
extern void disable_dev_wlc_ioctl(void);
	...
	wl_android_exit();
	//w00176398 cleanup wifi sta timer
	cleanup_wifi_sta_timer_module();
...
//end	
	wl_android_init();
	//w00176398 init wifi sta timer
	error = init_wifi_sta_timer_module();
	if(error != 0)
	{
		DHD_ERROR(("Init Wifi sta timer failed.\n"));
		cleanup_wifi_sta_timer_module();
	}
#ifdef DHDTHREAD
...
//w00176398
	do_gettimeofday(&end);
	timeuse_usec = (1000000*(end.tv_sec - start.tv_sec) + end.tv_usec) - start.tv_usec;
	printk("wifi driver use time: %d usec\n",timeuse_usec);
	timeuse_usec = (1000000*(end1.tv_sec - start.tv_sec) + end1.tv_usec) - start.tv_usec;
	printk("wifi driver download: %d usec\n",timeuse_usec);
//end
#ifdef DHD_DEBUG
	module_insert = 1;
#endif
