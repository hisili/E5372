 ...
 *
 * $Id: wl_cfg80211.c 347718 2012-07-27 21:27:50Z $
 */
	...
	u64 *cookie)
{
	...
	*cookie = id;
	...
	if (IS_P2P_SOCIAL(af_params->channel) &&
		...
		/* We need to retry Service discovery frames as they don't get retried
		  * immediately by supplicant
		  */
		if ((!ack) && (IS_GAS_REQ(sd_act_frm, action_frame->len))) {
			for (retry = 1; retry < retry_cnt; retry++) {
				WL_DBG(("Service Discovery action_frame retry %d len: %d chan %d"
					"category %d action %d\n",
	...
	if (wl_get_mode_by_netdev(wl, ndev) == WL_MODE_AP) {
		...
		if (wl_is_linkup(wl, e, ndev)) {
			...
			if (wl_get_drv_status(wl, CONNECTED, ndev)) {
				...
				printk("link down, call cfg80211_disconnected " MACSTR "\n",
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (event == WLC_E_ACTION_FRAME_RX) {
		...
		if (act_frm && (act_frm->subtype == P2P_PAF_GON_RSP)) {
			...
			wl_notify_escan_complete(wl, wl_to_p2p_bss_ndev(wl, P2PAPI_BSSCFG_DEVICE),
				true, true);
		}
	} else {
...
#undef LOCAL_BUF_LEN
	...
	for (i = 0; i < dtoh32(list->count); i++) {
		...
		update = FALSE;
		ht40_allowed = FALSE;
		c = wl_chspec_driver_to_host(list->element[i]);
