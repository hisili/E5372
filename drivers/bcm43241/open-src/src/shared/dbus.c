 ...
 *
 * $Id: dbus.c 346294 2012-07-20 18:49:41Z $
 */
...
typedef struct dbus_info {
    ...
    struct timer_list tx_restart_timer;
...
typedef struct
{
    struct timer_list rxq_check_timer;
    unsigned int txoff_drop_cnt;
    int netif_autostop;
}v7_dbus_ranger_t;

extern dbus_info_t* g_dbug_info;

v7_dbus_ranger_t v7_dbus_ranger;

int v7_dbus_ranger_init();
int rxq_check_timer_start(unsigned int secs);
void rxq_check_timeout(unsigned int secs);
void del_rxq_check_timer();
...
dbus_flowctrl_tx(void *dbi, bool on)
{
    ...
    if (g_txoff == ON) {
        mod_timer(&dbus_info->tx_restart_timer, jiffies + 1 * HZ);
    }
    else {
        mod_timer(&dbus_info->tx_restart_timer, jiffies + 3 * HZ);
    }
}
...
#ifdef __FreeBSD__
static int
dbus_irbrx_free_buf(dbus_info_t *dbus_info, dbus_irbq_t *q)
{
	dbus_irb_rx_t *irb;
	 ...
	 */
	while ((irb = (dbus_irb_rx_t*) q_deq(q)) != NULL) {
		if (irb->buf != NULL) {
			DBUSTRACE(("%s(): Free irbrx at 0x%x, size=0x%x\n",
				__FUNCTION__, (uint32) irb->buf, irb->buf_len));
			MFREE(dbus_info->pub.osh, irb->buf, irb->buf_len);
		}
	}
	return DBUS_OK;
}
...
static int
dbus_rxirbs_fill_one(dbus_info_t *dbus_info, dbus_irb_rx_t *rxirb)
{
	int err = DBUS_OK;
...
#ifndef __FreeBSD__
                refill_ret = dbus_rxirbs_fill(dbus_info);
                if(refill_ret != 0)
                {
                    if(rxirb_pending < 3)
                    {
                        rxq_check_timer_start(5);
                    }
                    else
                    {
                        rxq_check_timer_start(1 * HZ);
                    }
                }
                else
                {
                    rxq_check_timer_start(5 * HZ);                   
                }
#endif
...
#else // BCM_RPC_RXNOCOPY
            /* we need to release it if no device*/
            if (rxirb->pkt) {
                PKTFREE(dbus_info->pub.osh, rxirb->pkt, FALSE);
            }
...
#ifdef __FreeBSD__
		/* alway refill for only one case */
		dbus_rxirbs_fill_one(dbus_info, rxirb);
#endif
	} else {
...
#else // BCM_RPC_RXNOCOPY
        /* BEGIN: fuxiaowei - we need to release it if no device*/
        if (rxirb->pkt) {
            PKTFREE(dbus_info->pub.osh, rxirb->pkt, FALSE);
        }
        /* END: fuxiaowei */
...
/* BEGIN: fuxiaowei 
   we don't want to add lock to synchronize txoff operation.
   we think additional lock will slow down the speed.
   we don't want to change the logic alot.
   we only want to resolve the issue by one shot.
*/
static void tx_restart_timer_func(unsigned long data)
{
    dbus_info_t *dbus_info = (dbus_info_t *)data;
    int txirb_pending = 0;
    ...
    if (txirb_pending > (dbus_info->tx_low_watermark * 3)) {
        ...
        mod_timer(&dbus_info->tx_restart_timer, jiffies + 2 * HZ);

    }
    else {

        DHD_TRACE(("%s: 200: disable tx flow control\n", __FUNCTION__));

        dbus_info->txoff = OFF;

        if (dbus_info->cbs && dbus_info->cbs->txflowcontrol) 
        {
            dbus_info->cbs->txflowcontrol(dbus_info->cbarg, OFF);
        }
...
static void init_tx_restart_timer(dbus_info_t *dbus_info)
{
    if (dbus_info == NULL)
    {
        return;
    ...
    init_timer(&dbus_info->tx_restart_timer);
    dbus_info->tx_restart_timer.expires = jiffies + 600 * HZ; /* 600 seconds later */
    dbus_info->tx_restart_timer.data = (unsigned long)dbus_info;
    dbus_info->tx_restart_timer.function = tx_restart_timer_func;
    add_timer(&dbus_info->tx_restart_timer);
}

static void del_tx_restart_timer(dbus_info_t *dbus_info)
{
    if (dbus_info == NULL)
    {
        return;
    ...
    del_timer(&dbus_info->tx_restart_timer);

}

dbus_info_t* g_dbug_info = NULL;

void test_tx_restart_timer(int n)
{

    DHD_TRACE(("%s: 100 secs = %d\n", __FUNCTION__, n));
    ...
    g_dbug_info->txoff = ON;

    mod_timer(&g_dbug_info->tx_restart_timer, jiffies + n * HZ);
	...
	dbus_callbacks_t *cbs, dbus_extdl_t *extdl, struct shared_info *sh)
{
    ...
    g_dbug_info = dbus_info;
    init_tx_restart_timer(dbus_info);
...
dbus_detach(dbus_pub_t *pub)
{
	...
	if (dbus_info == NULL)
		return;
    del_tx_restart_timer(dbus_info);
	...
	if (dbus_info->rx_q) {
#ifdef __FreeBSD__
		dbus_irbrx_free_buf(dbus_info, dbus_info->rx_q);
#endif
...
#ifdef __FreeBSD__
int dbus_check_tx_nfree(dbus_pub_t *pub)
{
	dbus_info_t *dbus_info = (dbus_info_t *) pub;

	if (dbus_info->drvintf) {
		return dbus_info->drvintf->check_tx_nfree(dbus_info->bus_info);
	}
	return 0;
}
#endif
...
int v7_dbus_ranger_init()
{
    memset(&v7_dbus_ranger, 0 , sizeof(v7_dbus_ranger));
    
    init_timer(&v7_dbus_ranger.rxq_check_timer);

    v7_dbus_ranger.rxq_check_timer.function = rxq_check_timeout;    
    v7_dbus_ranger.rxq_check_timer.expires = jiffies + 200 * HZ;  
    v7_dbus_ranger.rxq_check_timer.data = 200;
    add_timer(&v7_dbus_ranger.rxq_check_timer);

    return 0;

}

int rxq_check_timer_start(unsigned int ms10_slot)
{

    v7_dbus_ranger.rxq_check_timer.data = ms10_slot;

    mod_timer(&v7_dbus_ranger.rxq_check_timer, jiffies + ms10_slot);
...
void rxq_check_timeout(unsigned int secs)
{
    ...
    dbus_info_t *dbus_info = g_dbug_info;
    int rxirb_pending;
    int ret = 0;

    if(dbus_info == NULL)
    {
        DHD_ERROR(("%s: 70: g_dbug_info is NULL\n", __FUNCTION__));    
        return;
    }

    //check watermark and call dbus_rxirbs_fill

    rxirb_pending = dbus_info->pub.nrxq - dbus_info->rx_q->cnt - 1;

    if ((rxirb_pending <= dbus_info->rx_low_watermark) && !dbus_info->rxoff) 
    {
        DHD_ERROR(("%s: 100: Low watermark so submit more %d <= %d \n", __FUNCTION__,
            dbus_info->rx_low_watermark, rxirb_pending));

        ret = dbus_rxirbs_fill(dbus_info);
    }

    //check water mark again and set timer

    if(ret == 0)
    {
        mod_timer(&v7_dbus_ranger.rxq_check_timer, jiffies + 5 * HZ);
    }
    else
    {
        DHD_ERROR(("%s: 200: ret = %d, rxirb_pending = %d\n", __FUNCTION__, ret, rxirb_pending));
        if(rxirb_pending < 3)
        {
            mod_timer(&v7_dbus_ranger.rxq_check_timer, jiffies + 5);
        }
        else
        {        
            mod_timer(&v7_dbus_ranger.rxq_check_timer, jiffies + 1 * HZ);
        }
    }
...
void del_rxq_check_timer()
{
    del_timer(&v7_dbus_ranger.rxq_check_timer);
}
//functions for ecall to debug

int force_rxirbs_fill()
{
    int ret = 0;
    ret = dbus_rxirbs_fill(g_dbug_info);

    printk("%s:100: ret = %d", __FUNCTION__, ret);
