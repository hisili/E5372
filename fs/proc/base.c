...
static int proc_tid_msa(struct task_struct *task, char *buffer)
{
    ...
    default:
        MSA_NOW(now);
        tp[msp1.cur_state] += msp1.last_change - now;
    }
    return
      sprintf(buffer,
          "State:      %s\n"        \
          ...
          (unsigned long)MSA_TO_NSEC(now),
          (unsigned long)MSA_TO_NSEC(tp[MSA_ONCPU_USER]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_ONCPU_SYS]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_INTERRUPTIBLE_SLEEP]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_UNINTERRUPTIBLE_SLEEP]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_INTERRUPTED]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_ONRUNQUEUE]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_STOPPED]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_ZOMBIE]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_POLL_SLEEP]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_PAGING_SLEEP]),
          (unsigned long)MSA_TO_NSEC(tp[MSA_FUTEX_SLEEP]));
}
