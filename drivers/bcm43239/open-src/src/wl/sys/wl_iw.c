...
//#define WL_ERROR printf x
#define WL_ERROR DHD_ERROR


#ifdef CSCAN
#define WL_TRACE  DHD_BTA
#define WL_ASSOC(x)
#define WL_INFORM DHD_GLOM
#define WL_WSEC(x)
#define WL_SCAN DHD_INTR
...
#include <wl_iw.h>
#ifdef CSCAN
static int wl_keep_alive(struct net_device *dev,
...
#if defined(OEM_ANDROID) || defined(CSCAN)
...
)
{
    ...
    if(dev_stats != NULL)
    {
		...
		p += snprintf(p, MAX_FLUX_LENGTH, "Rx-pkts: %lu %lu Tx-pkts: %lu %lu Tx-bytes: %lu %lu Rx-bytes: %lu %lu\n", 
		...
		WL_TRACE(("Rx-pkts: %lu %lu Tx-pkts: %lu %lu Tx-bytes: %lu %lu Rx-bytes: %lu %lu\n", 
...
)
{
    ...
    WL_TRACE(("%s:200 ssid: %s   SSID_len = %d\n", __FUNCTION__, ssid.SSID, ssid.SSID_len));        
	...
	if ((error = dev_wlc_ioctl(dev, WLC_REASSOC, &bssid, ETHER_ADDR_LEN))) {
		WL_ERROR(("%s: 300: WLC_REASSOC to %s failed \n", __FUNCTION__, ssid.SSID));
		return error;
	}
...
wl_iw_timerfunc(ulong data)
{
	...
	if (iscan) {
		...
		if (iscan->iscan_state != ISCAN_STATE_IDLE) {
            WL_TRACE(("timer trigger,abort the scan\n"));
...
wl_iw_create_wpaauth_wsec(struct net_device *dev)
{
	...
	if (iw->gcipher & (IW_AUTH_CIPHER_WEP40 | IW_AUTH_CIPHER_WEP104))
        ...
        /* Modified by benson for WPS under WPA_supplicant */
#ifdef CONFIG_WPS2	
        if (wsec == 0 && iw->privacy_invoked){      
            int terr = 0;
            
            WL_INFORM(("Trying wps ... \n"));
            if (( terr = dev_wlc_intvar_set(dev,"is_wps_enrollee",TRUE))) {
                WL_ERROR(("WPS configuration failed \n"));
            }
        } else if ( wsec ){ 
            int terr = 0;
            if (( terr = dev_wlc_intvar_set(dev,"is_wps_enrollee",FALSE))) {
                WL_ERROR(("WPS configuration failed clear \n"));
                return terr;
            }
        }
#else
...
)
{
	...
	case IW_AUTH_80211_AUTH_ALG:
		...
		WL_INFORM(("Setting the D11auth %x,alopen:%x\n", paramval,IW_AUTH_ALG_OPEN_SYSTEM));
                 ...
                 union iwreq_data *wrqu, char *ext)
{
	...
	str_ptr = extra;
    /* Added by benson for crash issue on APSTA */      
    if (iscan->iscan_state == ISCAN_STATE_SCANING) {
        WL_TRACE(("%s ISCAN already in progress \n", __FUNCTION__));
        goto exit_proc;
        }
...
)
{
	...
	if (dwrq->length && extra) {
		...
		{
			ret = wl_keep_alive(dev, info, (union iwreq_data *)dwrq, extra);
		}
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	switch (event_type) {
//#if defined(OEM_ANDROID) && defined(SOFTAP)
#if defined(CSCAN) && defined(SOFTAP)
	case WLC_E_PRUNE:
		if (ap_cfg_running) {
...
//#if defined(OEM_ANDROID) && defined(SOFTAP)
	...
	case WLC_E_ROAM:
		if (status != WLC_E_STATUS_SUCCESS) {
			WL_ERROR(("Roaming is not successful\n"));
#if 0
			roam_no_success++;
...
//#if defined(OEM_ANDROID) && defined(SOFTAP)
        ...
        WL_TRACE(("%s: 500: disconnect reason:%d\n", __FUNCTION__, strtoul(wrqu.addr.sa_data + ETHER_ADDR_LEN + 1,NULL, 0)));
		break;
	case WLC_E_AUTH:
		if((STA_DISCONNECT_REASON_CODE_AUTH_FAILED == (STA_DISCONNECT_REASON_CODE)reason) || 
			(STA_DISCONNECT_REASON_CODE_AUTH_WEP_NO_MATCH == (STA_DISCONNECT_REASON_CODE)reason))
		{
#if defined(SOFTAP)
			WL_SOFTAP(("STA disconnect received %d\n", event_type));
			if (ap_cfg_running) {
...
#endif 	
			WL_SOFTAP(("STA disconnect received reason %d\n", reason));
			...
			bzero(&extra, ETHER_ADDR_LEN);
	        	//w00176398 add for disconnect reason
	        	sprintf(wrqu.addr.sa_data + ETHER_ADDR_LEN + 1, "%d", reason);
	        	WL_TRACE(("%s: 600: disconnect reason111:%d\n", __FUNCTION__, strtoul(wrqu.addr.sa_data + ETHER_ADDR_LEN + 1,NULL, 0)));
		}
#endif
		break;
	case WLC_E_LINK:
	...
	default:
		...
		if (cmd) {
            WL_TRACE(("%s:2000: Event = %d, cmd = %x---------------\n", __FUNCTION__, event_type,cmd));
			if (cmd == SIOCGIWSCAN)
...
wl_pattern_atoh(char *src, char *dst)
{
	...
	if (strncmp(src, "0x", 2) != 0 &&
	    strncmp(src, "0X", 2) != 0) {
		WL_ERROR(("Mask invalid format. Needs to start with 0x\n"));
		return -1;
	}
	src = src + 2; /* Skip past 0x */
	if (strlen(src) % 2 != 0) {
		WL_ERROR(("Mask invalid format. Needs to be of even length\n"));
		return -1;
	}
	for (i = 0; *src != '\0'; i++) {
		char num[3];
...
static int
wl_keep_alive_action(struct net_device *dev, uint32 period_msec, uint8 *pktbuf)
{
	char 				*str;
	...
	bufptr = (char *)kmalloc(MAX_WLIW_IOCTL_LEN, GFP_KERNEL);
	if (bufptr == NULL) {
		WL_ERROR(("%s: malloc ioctl buf fail\n", __FUNCTION__));
		return -1;
	}
	memset(bufptr, 0, MAX_WLIW_IOCTL_LEN);
	/*
	** Set keep-alive attributes.
	*/
	str = "keep_alive";
	str_len = strlen(str);
	strncpy(bufptr, str, str_len);
	bufptr[ str_len ] = '\0';

	keep_alive_pktp = (wl_keep_alive_pkt_t *) (bufptr + str_len + 1);
	keep_alive_pkt.period_msec = period_msec;
	buf_len = str_len + 1;
	...
	if (keep_alive_pkt.period_msec == 0) {
		...
		keep_alive_pkt.len_bytes =
		        htod16(wl_pattern_atoh((char *)pktbuf, (char *) keep_alive_pktp->data));
	...
	rc = dev_wlc_ioctl(dev, WLC_SET_VAR, bufptr, buf_len);
	if (bufptr != NULL) {
		kfree(bufptr);
	}
...
{	
	...
	uint32 time_int;
	uint8 *packet;
	char *p_extra=extra+strlen("KEEP_ALIVE")+1;
	WL_TRACE(("\n%s: extra val start: \n",__FUNCTION__));
	for(i=0;i<(wrqu->data.length-strlen("KEEP_ALIVE"));i++)
	{	
		if(*(p_extra + i) == ' ')
		{
		   packet = p_extra + i + 1;
		   WL_TRACE(("8888 packet length=%d, val = %s\n",strlen(packet),packet));
		   break;		
		}
		time_str[i] = *(p_extra+i);
		//WL_TRACE(("6666extra val = %c: ",time_str[i]));		
	}

	time_int = bcm_atoi(time_str);
	WL_TRACE(("\n %s: extra val end,time_int=%d \n",__FUNCTION__,time_int));

	err=wl_keep_alive_action(dev, time_int, packet);
	return err;
}
#endif
