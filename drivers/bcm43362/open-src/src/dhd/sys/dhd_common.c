...
#endif
int dhd_msg_level = DHD_ERROR_VAL |DHD_TRACE_VAL|DHD_INFO_VAL |DHD_DATA_VAL|DHD_CTL_VAL|DHD_TIMER_VAL| DHD_HDRS_VAL| DHD_BYTES_VAL | DHD_INTR_VAL| DHD_GLOM_VAL|DHD_EVENT_VAL| DHD_BTA_VAL| DHD_ISCAN_VAL;

//w00176398 add interface for setting bcm debug level
//BEGIN
void SetDHDMsgLevel(unsigned int level)
{
	printk("Old DHD Level:0x%x New DHD Level:0x%x\n",dhd_msg_level,level);	
	dhd_msg_level = level;
}

EXPORT_SYMBOL(SetDHDMsgLevel);
//END
...
#if 1
	dhd_msg_level = DHD_ERROR_VAL |DHD_EVENT_VAL /*| DHD_DATA_VAL*/;
#if 1 && defined(GLOM_CODES) && defined(BALONG_PLATFORM)
	dhd_msg_level |= DHD_TXGLOM_VAL;
#endif

#else
	dhd_msg_level = DHD_ERROR_VAL |DHD_TRACE_VAL|DHD_INFO_VAL |DHD_DATA_VAL|DHD_CTL_VAL|DHD_TIMER_VAL| DHD_HDRS_VAL| DHD_BYTES_VAL | DHD_INTR_VAL| DHD_GLOM_VAL|DHD_EVENT_VAL| DHD_BTA_VAL| DHD_ISCAN_VAL;
...
wl_show_host_event(wl_event_msg_t *event, void *event_data)
{
	...
	uchar *buf;
	char err_msg[512], eabuf[ETHER_ADDR_STR_LEN];
	...
	if (datalen) {
		int pos = 0;
		buf = (uchar*) event_data;
		memset(err_msg, 0x0, 511);
		pos += snprintf(err_msg+pos, 511, " data (%d) : ", datalen);
		for (i = 0; i < datalen; i++)
			pos += snprintf(err_msg+pos, 511, " 0x%02x ", *buf++);
		pos += snprintf(err_msg+pos,  511, "\n");
		err_msg[511] = '\0';
		DHD_EVENT(("%s", err_msg));
	}
#endif
			...
			if (ifevent->ifidx > 0 && ifevent->ifidx < DHD_MAX_IFS) {
				if (ifevent->action == WLC_E_IF_ADD) {
					...
					if (!ap_net_dev) {
						...
						DHD_ERROR(("%s: ap_net_dev is not null\n", __FUNCTION__));
					}
#endif
				} else
...
dhd_preinit_ioctls(dhd_pub_t *dhd)
{
	...
	char iovbuf[256 + 12];	/*  Room for "event_msgs" + '\0' + bitvec  */
...
#if 1
	{
		uint32 glompktnum = 10;
		uint32 glomtxdelay = 2;
		uint32 glomtxmode = 7; // timer/pkt_number
		uint32 glom = 0;
		uint32 dongle_align = DHD_SDALIGN;

		glom = 1;

		DHD_EVENT(("%s=======enable DHD_TX_GLOM=========\n", __FUNCTION__));

		/* Match Host and Dongle rx alignment */
		bcm_mkiovar("bus:txglomalign", (char *)&dongle_align, 4, iovbuf, sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);

		/* enable glom option per default */
		bcm_mkiovar("bus:txglom", (char *)&glom, 4, iovbuf, sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);


		bcm_mkiovar("bus:maxtxpktglom", (char *)&glompktnum, 4, iovbuf, sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);

		bcm_mkiovar("bus:txlazydelay", (char *)&glomtxdelay, 4, iovbuf, sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);

		/* Set txmode */
		bcm_mkiovar("bus:txglom", (char *)&glomtxmode, 4, iovbuf, sizeof(iovbuf));
		dhd_wl_ioctl_cmd(dhd, WLC_SET_VAR, iovbuf, sizeof(iovbuf), TRUE, 0);

		//w00176398 delete 2011-11-29
		//DHD_EVENT(("%s=======enabled inchip-debug=========\n", __FUNCTION__));
		//dhd_serialconsole(dhd->bus, TRUE, TRUE, &bcmerror);
	}
#endif

#else

	DHD_EVENT(("%s=======disable DHD_TX_GLOM=========\n", __FUNCTION__));
...
#endif
#ifdef BALONG_PLATFORM
	setbit(eventmask, WLC_E_ROAM);
	setbit(eventmask, WLC_E_ROAM_START);
	setbit(eventmask, WLC_E_PFN_NET_FOUND);
	setbit(eventmask, WLC_E_PFN_NET_LOST);
	setbit(eventmask, WLC_E_RESET_COMPLETE);
	setbit(eventmask, WLC_E_UNICAST_DECODE_ERROR);
	setbit(eventmask, WLC_E_MULTICAST_DECODE_ERROR);
	setbit(eventmask, WLC_E_CHANNEL_ADOPTED);
	setbit(eventmask, WLC_E_AP_STARTED);
	setbit(eventmask, WLC_E_FIFO_CREDIT_MAP);
	setbit(eventmask, WLC_E_DFS_AP_STOP);
	setbit(eventmask, WLC_E_DFS_AP_RESUME);
#endif /* BALONG_PLATFORM */
