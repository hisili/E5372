 ...
 *
 * $Id: wlu_linux.c 344514 2012-07-13 00:31:00Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
int    g_iConsoleFd = 0;
int    g_iAtDevFd = 0;
char   wifi_at_cmdbuf[WIFI_AT_CMDBUF_LEN] = "";
...
main(int argc, char **argv)
{
...
#if defined (WIFI_AT_INTERFACE)
    g_iConsoleFd = open(WIFI_CONSOLE_DEVICE,O_RDWR);
    ...
    if(g_iConsoleFd == 0)
    {
        ...
        g_iAtDevFd = open(WIFI_AT_DEVICE,O_RDWR);
        if(g_iAtDevFd)
        {
            write(g_iAtDevFd,WL_CMD_START, sizeof(WL_CMD_START));
        }
        else
        {
            printf("can't open %s\n",WIFI_CONSOLE_DEVICE);
