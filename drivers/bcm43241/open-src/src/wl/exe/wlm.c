 ...
 *
 * $Id: wlm.c 309193 2012-01-19 00:03:57Z $
 */
...
int wlmEstimatedPowerGet(int *estPower, int chain)
{
	...
	if (ioctl_version == 1) {
		is2g = LCHSPEC_IS2G(power.chanspec);
	} else {
		is2g = CHSPEC_IS2G(power.chanspec);
	}
...
int wlmRssiGet(int *rssi)
{
	...
	if (wlu_var_getbuf_med(irh, "pkteng_stats", NULL, 0, (void **)&cnt)) {
		printf("wlmRssiGet: %s\n", wlmLastError());
		return FALSE;
	}
