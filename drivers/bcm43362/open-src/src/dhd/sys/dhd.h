 ...
 *
 * $Id: dhd.h 278821 2011-08-20 00:26:12Z $
 */
...
typedef enum flux_timer_flag
{
    FLUX_NO_START_TIMER = 0,
    FLUX_DELETED_TIMER,
    FLUX_STARTED_TIMER,
}FLUX_TIMER_FLAG;

//w00176398 add for wifi sta timeout begin
typedef struct sta_flux
{
    struct timer_list flux_timer; //timer
    unsigned int disconnect_time; //minutes
    unsigned long last_net_time;  //last rx/tx time
    spinlock_t flux_spinlock;     //spin lock
    struct mutex flux_mutex;      //mutex
    bool is_sta_connect;         //timer flag
}STA_FLUX;

#define WIFI_STA_SPIN_LOCK(spinLock) spin_lock_bh(&(spinLock))
#define WIFI_STA_SPIN_UNLOCK(spinLock) spin_unlock_bh(&(spinLock))

extern bool check_wifi_sta_idle_timeout();
extern void set_wifi_sta_last_net_time(unsigned long time);
extern bool check_sta_package(index);
extern bool check_sta_connect_status();

#define IS_WIFI_STA_IDLE_TIMEOUT() check_wifi_sta_idle_timeout()
#define SET_WIFI_STA_LAST_NET_TIME(time) set_wifi_sta_last_net_time(time)
#define IS_STA_PACKAGE(index) check_sta_package(index)
#define IS_STA_CONNECTED() check_sta_connect_status()

int init_wifi_sta_timer_module();
void cleanup_wifi_sta_timer_module();
//w00176398 add for wifi sta timeout end
...
typedef struct dhd_pub {
	...
	/* For supporting multiple interfaces */
	dngl_stats_t dstats[DHD_MAX_IFS];	/* Stats for dongle-based data */
#else
