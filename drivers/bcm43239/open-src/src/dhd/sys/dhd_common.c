              ...
              wl_event_msg_t *event, void **data_ptr)
{
	...
	*data_ptr = &pvt_data[1];
	...
	#if defined(DHD_UPDATE_INTF_MAC)
	#if 0   /* BEGIN added by wangweichao to make the ap and sta have different MAC*/
	if (*ifidx > 0 && WLC_E_LINK == type && 0 == dhd_ap_mode )
	#endif
	if (*ifidx > 0 && WLC_E_LINK == type)     /*END added by wangweichao to make the ap and sta have different MAC*/
	{
		if ( flags & WLC_EVENT_MSG_LINK)
		{
			dhd_update_mac_if(dhd_pub->info, *ifidx);
		}
	}
	#endif
    ...
    {
        uint type_tmp;
        type_tmp = ntoh32(event->event_type);
        printk(KERN_ERR "%s: 1000: event type = %u\n",  __FUNCTION__, type_tmp);
    }
	return (BCME_OK);
}
