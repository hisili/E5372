...
#include "balong_core_hi6920csp500.h"


static irqreturn_t balong_timer_irq_handler(int irq, void *dev_id)
{
    unsigned int readValue;
    readValue = readl(__io_address(PV500V1_TIMER4_BASE) + CLK_REGOFF_CLEAR);
	timer_tick();
	return IRQ_HANDLED;
}
...
{
    unsigned int tc;
    writel(0x02010101,__io_address(PBXA9_GIC_CPU_BASE)+0x1824);
    writel(CLK_DEF_DISABLE, __io_address(PV500V1_TIMER4_BASE) + CLK_REGOFF_CTRL);
    tc = (SYS_TIMER_CLK / sysClkTicksPerSecond) - AMBA_RELOAD_TICKS;
    writel(tc, __io_address(PV500V1_TIMER4_BASE) + CLK_REGOFF_LOAD);
    writel(CLK_DEF_ENABLE, __io_address(PV500V1_TIMER4_BASE) + CLK_REGOFF_CTRL);
    
	setup_irq (INT_LVL_TIMER_4567, &balong_timer_irq);
}
