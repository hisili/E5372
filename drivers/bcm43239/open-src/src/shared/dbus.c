		...
		if (status == DBUS_OK) {
			if ((rxirb->buf != NULL) && (rxirb->actual_len > 0)) {
				...
				if (dbus_info->cbs && dbus_info->cbs->recv_pkt)
					...
					rxirb->actual_len); /* fuxiaowei - add len */
			} else {
				ASSERT(0); /* Should not happen */
			}
			...
			if ((rxirb_pending <= dbus_info->rx_low_watermark) &&
                ...
                DBUSTRACE(("%s: 1100: Low watermark so submit more %d <= %d \n", __FUNCTION__, dbus_info->rx_low_watermark, rxirb_pending));
...
static void tx_restart_timer_func(unsigned long data)
{
	...
	if (txirb_pending > (dbus_info->tx_low_watermark * 3)) {
		...
		WIFI_TRACE(("%s: 200: disable tx flow control\n", __FUNCTION__));
...
//timer to check whether



//#define v7_dbus_ranger.dhd v7_dbus_ranger.dhdp->info
...
void rxq_check_timeout(unsigned int secs)
{
    ...
    if(dbus_info == NULL)
    {
        WIFI_DETAIL(("%s: 70: g_dbug_info is NULL\n", __FUNCTION__));    
        return;
    }
    ...
    {
    	WIFI_DETAIL(("%s: 200: ret = %d, rxirb_pending = %d\n", __FUNCTION__, ret, rxirb_pending));
    	if(rxirb_pending < 3)
    	{
