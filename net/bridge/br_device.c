...
netdev_tx_t br_dev_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	//printk("br->port_count: %d\n", br->port_count);

	/* fuxiaowei - if there is only one port in the bridge, we send it directly */
	if (br->port_count == 1) {
		...
		if (p->state == BR_STATE_FORWARDING) {
			...
			dev_hold(dstdev);
			dstdev->netdev_ops->ndo_start_xmit(skb, dstdev);
			dev_put(dstdev);
...
#if(defined(CHIP_BB_6920ES) || defined (CHIP_BB_6920CS))
	/* f00198480: here we need to add additional 16 bytes for HSIC WIFI to avoid copying data */
	dev->hard_header_len = ETH_HLEN + 16;
#endif	
    dev->features = NETIF_F_NO_CSUM | NETIF_F_NETNS_LOCAL;
