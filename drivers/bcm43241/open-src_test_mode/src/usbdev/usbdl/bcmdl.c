 ...
 *
 * $Id: bcmdl.c 336855 2012-06-05 12:10:34Z $
 */
...
main(int argc, char **argv)
{
	...
	rdl_state_t rdl;
	bootrom_id_t id;
	int started = 0, cont = 0, reboot = 0, reset = 0, dl_go = 1, err_inj = 0,dl_sleep=0;
...
start:
	...
	if (rdl.state == DL_RUNNABLE && dl_go) {
		if(dl_sleep ){
			printf(" FW download completed, do everything what you want before run it.\n");
			sleep(dl_sleep);
		};
