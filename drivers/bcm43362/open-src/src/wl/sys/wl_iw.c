...
#define MAX_WIFI_STA_TIMEOUT_LENGTH 128
#define WL_TR(args) printk args
#define DEVICE_ID_WAN_WIFI 11
#define WAN_DIALRPT_DOWN_F_WIFI 0x0601

#define DEFAULT_STA_DISCONNECT_TIME 0
#define NO_FLUX_EXPIRES (g_sta_flux.disconnect_time*HZ + g_sta_flux.last_net_time)

char wifi_sta_timeout[MAX_WIFI_STA_TIMEOUT_LENGTH] = {'\0'};
static struct proc_dir_entry *proc_entry = NULL;
static struct proc_dir_entry *root_entry = NULL;
static struct proc_dir_entry *subroot_entry = NULL;
STA_FLUX g_sta_flux;
static char* msg = "NO FLUX"; 
...
#define	wl_trace_err(fmt, args...)	 wifi_trace_add_exp(WL_ID_DEBUG,OM_DEBUG_ERROR,fmt, ## args) 
#define	wl_trace_dbg(fmt, args...)	 wifi_trace_add_exp(WL_ID_DEBUG,OM_DEBUG_DEBUG,fmt, ## args) 
#define	wl_trace_info(fmt, args...)	 wifi_trace_add_exp(WL_ID_DEBUG,OM_DEBUG_INFO,fmt, ## args) 

#define WL_CHECK_ERROR()   wifi_trace_check_lev(WL_ID_DEBUG,OM_DEBUG_ERROR)
#define WL_CHECK_DEBUG()   wifi_trace_check_lev(WL_ID_DEBUG,OM_DEBUG_DEBUG)
#define WL_CHECK_INFO()    wifi_trace_check_lev(WL_ID_DEBUG,OM_DEBUG_INFO)
#endif

#ifdef BALONG_WIFI_TRACE
#define WL_ERROR(args)    do { if (WL_CHECK_ERROR()) wl_trace_err args;} while (0)
#define WL_TRACE(args)    do { if (WL_CHECK_INFO())  wl_trace_info args;} while (0)
#define WL_ASSOC(args)    do { if (WL_CHECK_DEBUG()) wl_trace_dbg args;} while (0)
#define WL_INFORM(args)   do { if (WL_CHECK_DEBUG()) wl_trace_dbg args;} while (0)
#define WL_WSEC(args)     do { if (WL_CHECK_DEBUG()) wl_trace_dbg args;} while (0)
#define WL_SCAN(args)	  do { if (WL_CHECK_DEBUG()) wl_trace_dbg args;} while (0)
#else
#define WL_ERROR(x)		do { printf x; } while(0)
#define WL_TRACE(x)		do { printf x; } while(0)
#define WL_ASSOC(x)		do { printf x; } while(0)
#define WL_INFORM(x)	do { printf x; } while(0)
#define WL_WSEC(x)		do { printf x; } while(0)
#define WL_SCAN(x)		do { printf x; } while(0)
#endif


#ifdef BALONG_WIFI_TRACE

#define WL_PNO(x)		do { if (WL_CHECK_DEBUG()) wl_trace_dbg x;} while (0)

#else

#ifdef PNO_SET_DEBUG
#define WL_PNO(x)		do { printf x; } while(0)
#else
#define WL_PNO(x)		do { printf x; } while(0)
#endif
...
#define WL_TRACE_COEX(x)    do { if (WL_CHECK_DEBUG()) wl_trace_dbg x;} while (0)
#define WL_TRACE_SCAN(x)    do { if (WL_CHECK_DEBUG()) wl_trace_dbg x;} while (0)
...
#ifdef WL_CFG80211
void wl_cfg80211_set_dongle_down(void);
#endif
...
#endif 
//w00176398 add for wifi-offload begin
bool check_wifi_sta_idle_timeout()
{
	bool bRet = false;
	
	WL_TRACE(("%s dis_time:%d current_time:%d last_net_time:%d\n",__func__,g_sta_flux.disconnect_time,jiffies,g_sta_flux.last_net_time));
	bRet = g_sta_flux.disconnect_time*HZ > (jiffies-g_sta_flux.last_net_time) ?false:true;

	return bRet;
}

void set_wifi_sta_last_net_time(unsigned long time)
{
	WL_TRACE(("%s last_net_time:%d new time:%d\n",__func__,g_sta_flux.last_net_time,time));
	g_sta_flux.last_net_time = time;
}

bool check_sta_package(index)
{
	if(0 == index)
	{
		return true;
...
bool check_sta_connect_status()
{
	WL_TRACE(("%s sta status:%d\n",__func__, g_sta_flux.is_sta_connect));
	return g_sta_flux.is_sta_connect;
}



void wifi_idle_timer_handler(unsigned long lparam)
{
	char* str = (char*)lparam; 
	long unsigned  int diffTimer;

	if(!IS_STA_CONNECTED())
	{
		WL_TRACE(("timer timeout!!!,but sta is disconnected,don't send event to web sdk \n"));
		return;
	}
	
	if(!IS_WIFI_STA_IDLE_TIMEOUT())
	{
		WL_TRACE(("%s %lu last net time %lu interval:%d\n",__func__,jiffies, 
			g_sta_flux.last_net_time,(jiffies-g_sta_flux.last_net_time)));
		diffTimer = NO_FLUX_EXPIRES;
		mod_timer(&g_sta_flux.flux_timer, diffTimer);
	}
	else
	{     
		del_timer(&g_sta_flux.flux_timer);
		int ret;
		DEVICE_EVENT event;
		event.device_id = DEVICE_ID_WAN_WIFI;
		event.event_code = WAN_DIALRPT_DOWN_F_WIFI;
		event.len = 0;

		ret = device_event_report(&event, sizeof(DEVICE_EVENT));
		if (-1 == ret) {
			WL_ERROR(("device_event_init: can't add event\n"));
		}

		WL_TRACE(("timeout send event to web sdk\n"));
	}
}

void wifi_idle_timer_func(unsigned long lparam)
{ 
	WL_TRACE(("%s %lu last net time %lu interval:%d\n",__func__,jiffies, 
		g_sta_flux.last_net_time,(jiffies-g_sta_flux.last_net_time)));
	
	//WIFI_STA_SPIN_LOCK(g_sta_flux.flux_spinlock);
	wifi_idle_timer_handler(lparam);
	//WIFI_STA_SPIN_UNLOCK(g_sta_flux.flux_spinlock);
}

static int wifi_idle_timer_start(unsigned int seconds)
{
	long unsigned  int diffTimer;
	//reset timer
	SET_WIFI_STA_LAST_NET_TIME(jiffies);
	if(seconds <= 0 || !IS_STA_CONNECTED())
	{
		WL_TRACE(("Interval time(%d) is less than 0 Or sta don't connect, don't start timer\n",seconds));
		return 1;
	}
	
	WL_TRACE(("%s current:%d last timer:%d\n",__func__,jiffies, g_sta_flux.last_net_time));
	//if(!IS_WIFI_STA_IDLE_TIMEOUT())
	{
		diffTimer = NO_FLUX_EXPIRES;
		mod_timer(&g_sta_flux.flux_timer, diffTimer);
...
void wifi_idle_timer_init()
{
	init_timer(&g_sta_flux.flux_timer);

	WL_TRACE(("%s current:%d last timer:%d\n",__func__,jiffies, g_sta_flux.last_net_time));
	g_sta_flux.flux_timer.function = wifi_idle_timer_func; 
	g_sta_flux.flux_timer.data = (unsigned long) msg;
}

void wifi_idle_timer_delete()
{
	WL_TRACE(("%s\n",__func__));
	del_timer(&g_sta_flux.flux_timer);
}

ssize_t wifi_sta_timeout_write( struct file *filp, const char __user *buff,
                        unsigned long len, void *data )
{
	if (len >= MAX_WIFI_STA_TIMEOUT_LENGTH) 
	{
		WL_ERROR(("wifi sta timeout: len(%d) is large!\n",len));
		return -ENOSPC;
	}
	WL_TRACE(("%s begin %s\n",__func__,wifi_sta_timeout));
	if (copy_from_user( wifi_sta_timeout, buff, len )) 
	{
		return -EFAULT;
	}
	
	wifi_sta_timeout[len] = '\0';
	g_sta_flux.disconnect_time = bcm_atoi(wifi_sta_timeout) > 0? bcm_atoi(wifi_sta_timeout):0;
	//WIFI_STA_SPIN_LOCK(g_sta_flux.flux_spinlock);
	wifi_idle_timer_start(g_sta_flux.disconnect_time);
	//WIFI_STA_SPIN_UNLOCK(g_sta_flux.flux_spinlock);

	WL_TRACE(("%s end %s %d len:%d\n",__func__,wifi_sta_timeout,bcm_atoi(wifi_sta_timeout),len));
	return len;
}

int wifi_sta_timeout_read( char *page, char **start, off_t off,
...
int init_wifi_sta_idle_timeout( void )
{
	int ret = 0;

	memset( wifi_sta_timeout, 0, MAX_WIFI_STA_TIMEOUT_LENGTH );
	root_entry = proc_mkdir("dial", NULL);
	if (root_entry == NULL) 
	{
		WL_ERROR(("Couldn't create proc/dial\n"));
		return -1;
	}
	
	subroot_entry = proc_mkdir("wifi", root_entry);
	if (subroot_entry == NULL) 
	{
		WL_ERROR(("Couldn't create proc/dial/wifi\n"));
		return -1;
	}
	
	proc_entry = create_proc_entry( "idle_timeout", 0644, subroot_entry );
	if (proc_entry == NULL) 
	{
		WL_ERROR(("wifi_sta_idle_timeout: Couldn't create proc entry\n"));
		return -1;
	} 
	else 
	{
		proc_entry->read_proc = wifi_sta_timeout_read;
		proc_entry->write_proc = wifi_sta_timeout_write;
		WL_TRACE(("wifi_sta_timeout: Module loaded.\n"));
	}

	return ret;
}

void cleanup_wifi_sta_idle_timeout( void )
{
	if(subroot_entry != NULL)
	{
		remove_proc_entry("idle_timeout", subroot_entry);
		subroot_entry = NULL;
	}	
	if(root_entry != NULL)
	{
		remove_proc_entry("wifi", root_entry);
		root_entry = NULL;
		remove_proc_entry("dial", NULL);
	}
	WL_TRACE(("wifi_sta_idle_timeout: Module unloaded.\n"));
}

int init_wifi_sta_timer_module()
{
	int ret = 0;
	
	spin_lock_init(&g_sta_flux.flux_spinlock);
	g_sta_flux.disconnect_time = 0;
	g_sta_flux.is_sta_connect = false;
	g_sta_flux.last_net_time = jiffies;
	ret = init_wifi_sta_idle_timeout();
	wifi_idle_timer_init();
	
	return ret;
}

void cleanup_wifi_sta_timer_module()
{
	wifi_idle_timer_delete();
	cleanup_wifi_sta_idle_timeout();
}
//w00176398 add for wifi-offload end
static void
swap_key_from_BE(
	wl_wsec_key_t *key
)
...
)
{
	...
	if (dev_wlc_ioctl_off)
	{
        ...
        WL_TRACE(("%s: module removing. skip it.\n", __FUNCTION__));
		return -ENODEV;
	}
...
)
{
	...
	//p += snprintf(p, MAX_WX_STRING, "%s rssi %d noise %d", ssidbuf, rssi, 0);
	p += snprintf(p, MAX_WX_STRING, "rssi %d noise %d\n",rssi, 0);
	wrqu->data.length = p - extra + 1;
...
//w00176398
static unsigned long g_ulSumRxPkts = 0;
static unsigned long g_ulSumTxPkts = 0;
static unsigned long g_ulSumTxBytes = 0;
static unsigned long g_ulSumRxBytes = 0;

static unsigned long g_ulCurRxPkts = 0;
static unsigned long g_ulCurTxPkts = 0;
static unsigned long g_ulCurTxBytes = 0;
static unsigned long g_ulCurRxBytes = 0;

static unsigned long g_ulCarryRxPkts = 0;
static unsigned long g_ulCarryTxPkts = 0;
static unsigned long g_ulCarryTxBytes = 0;
static unsigned long g_ulCarryRxBytes = 0;
#define MAX_FLUX_LENGTH 256
...
)
{
    ...
    {   
        dev_stats = ops->ndo_get_stats(dev);
    }
    else
    {
    ...
    if(dev_stats != NULL)
    {
        RxPkts = dev_stats->rx_packets;
        TxPkts = dev_stats->tx_packets;
        RxBytes = dev_stats->rx_bytes;
        TxBytes = dev_stats->tx_bytes;

		RxPkts = (RxPkts >= g_ulCurRxPkts)?(RxPkts - g_ulCurRxPkts):(RxPkts + (~g_ulCurRxPkts) + 1);
		TxPkts = (TxPkts >= g_ulCurTxPkts)?(TxPkts - g_ulCurTxPkts):(TxPkts + (~g_ulCurTxPkts) + 1);
		RxBytes = (RxBytes >= g_ulCurRxBytes)?(RxBytes - g_ulCurRxBytes):(RxBytes + (~g_ulCurRxBytes) + 1);
		TxBytes = (TxBytes >= g_ulCurTxBytes)?(TxBytes - g_ulCurTxBytes):(TxBytes + (~g_ulCurTxBytes) + 1);

		if(RxPkts < g_ulSumRxPkts)
		{
			g_ulCarryRxPkts++;
		}
		if(TxPkts < g_ulSumTxPkts)
		{
			g_ulCarryTxPkts++;
		}
		if(RxBytes < g_ulSumRxBytes)
		{
			g_ulCarryRxBytes++;
		}
		if(TxBytes < g_ulSumTxBytes)
		{
			g_ulCarryTxBytes++;
		}

        g_ulSumRxPkts = RxPkts;
        g_ulSumTxPkts = TxPkts;
        g_ulSumRxBytes = RxBytes;
        g_ulSumTxBytes = TxBytes;
        
		p += snprintf(p, MAX_FLUX_LENGTH, "Rx-pkts: %lu %u Tx-pkts: %lu %u Tx-bytes: %lu %u Rx-bytes: %lu %u\n", 
		                       g_ulCarryRxPkts, g_ulSumRxPkts, g_ulCarryTxPkts, g_ulSumTxPkts, 
		                       g_ulCarryTxBytes, g_ulSumTxBytes, g_ulCarryRxBytes, g_ulSumRxBytes);
		WL_TRACE(("Rx-pkts: %lu %u Tx-pkts: %lu %u Tx-bytes: %lu %u Rx-bytes: %lu %u\n", 
		                       g_ulSumRxPkts, g_ulCarryRxPkts, g_ulSumTxPkts, g_ulCarryTxPkts, 
		                       g_ulSumTxBytes, g_ulCarryTxBytes, g_ulSumRxBytes, g_ulCarryRxBytes));
	}
	else
	{
		WL_ERROR(("get flux info failed!!!\n"));   
    }
...
)
{
    ...
    if(dev_stats != NULL)
    {
        g_ulCurRxPkts = dev_stats->rx_packets;
        g_ulCurTxPkts = dev_stats->tx_packets;
        g_ulCurTxBytes = dev_stats->tx_bytes;
        g_ulCurRxBytes = dev_stats->rx_bytes;
        WL_TRACE(("Rx-pkts: %lu %u Tx-pkts: %lu %u Tx-bytes: %lu %u Rx-bytes: %lu %u", 
                                   dev_stats->rx_packets, 0, dev_stats->tx_packets, 0, 
                                   dev_stats->tx_bytes, 0, dev_stats->rx_bytes, 0));
    }
    else
    {
		WL_ERROR(("clear flux info failed!!!\n"));   
    }
...
void wl_iw_check_apasta_concurrent(struct net_device *dev)
{
	if ( apsta_enable && ap_net_dev ) {
		...
		WL_TRACE(("%s: stop the ap part of apsta concurrent\n", __FUNCTION__));
...
{
    printk("dis time:%d new dis time:%d\n",g_sta_flux.disconnect_time,time);
    g_sta_flux.disconnect_time = time;
}
//w00176398
/*no flux disconnect event*/
#define SIOCSIWNOFLUX	0x8B40		/* no flux disconnect event */
//w00176398 no flux disconnect event
static int
wl_iw_send_no_flux_event(struct net_device *dev, char* timer)
{
    /*
	union iwreq_data wrqu;

	memset(&wrqu, 0, sizeof(wrqu));
	printk("wl_iw_send_no_flux_event begin\n");
	wireless_send_event(dev, SIOCSIWNOFLUX, &wrqu, NULL);
	printk("wl_iw_send_no_flux_event end\n");
	*/
	union iwreq_data wrqu;
	char extra[IW_CUSTOM_MAX + 1];
	int cmd;
	...
	cmd = IWEVCUSTOM;
	memset(&wrqu, 0, sizeof(wrqu));
	if (strlen("NO_FLUX") > sizeof(extra))
		return -1;

	strcpy(extra, "NO_FLUX");
	wrqu.data.length = strlen(extra);
    WL_TRACE(("wl_iw_send_no_flux_event begin\n"));
	wireless_send_event(dev, cmd, &wrqu, extra);
	net_os_wake_lock_timeout_enable(dev);
	WL_TRACE(("wl_iw_send_no_flux_event end\n"));
	...
	g_sta_flux.disconnect_time = bcm_strtoul(timer, NULL, 10);
	WL_TRACE(("disconnect time:%d\n",g_sta_flux.disconnect_time));
	return 0;
}
...
wl_iw_combined_scan_set(struct net_device *dev, wlc_ssid_t* ssids_local, int nssid, int nchan)
{
	...
	if (nssid > 0) {
		...
		if (i + nssid * sizeof(wlc_ssid_t) > params_size) {
			...
			WL_TRACE(("additional ssids exceed params_size\n"));
			err = -1;
			goto exit;
		}
...
)
{
    ...
    WL_TRACE(("add %s: SIOCSIWPRIV request info->cmd:%x, info->flags:%d dwrq->length:%d\n",
		dev->name, info->cmd, info->flags, dwrq->length));
    ...
    WL_TRACE(("add %s: SIOCSIWPRIV request %s, info->cmd:%x, info->flags:%d\n dwrq->length:%d\n",
	...
	if (dwrq->length && extra) {
        ...
        //w00176398 add for getting flux
        else if (strnicmp(extra, "FLUX", strlen("FLUX")) == 0)
			ret = wl_iw_get_flux(dev, info, (union iwreq_data *)dwrq, extra);
        //w00176398 add for getting flux
        else if (strnicmp(extra, "CLEAR-FLUX", strlen("CLEAR-FLUX")) == 0)
			ret = wl_iw_clear_flux(dev, info, (union iwreq_data *)dwrq, extra);
        else if (strnicmp(extra, "NO_FLUX", strlen("NO_FLUX")) == 0)
		{
		    WL_TRACE(("no_flux cmd:%s\n",extra));
		    ret = wl_iw_send_no_flux_event(dev,extra + strlen("NO_FLUX") + 1);
        }
		...
		else if (strnicmp(extra, "P2P_MPC_SET", strlen("P2P_MPC_SET")) == 0) {
			wl_iw_set_p2p_mpc(dev, info, (union iwreq_data *)dwrq, extra);
	    }
#endif
	    else {
...
int wl_iw_event_debug = 0;

void set_wl_iw_event_flag(int flag)
{
    printf("Ori wl_iw_event_flag:0x%x New wl_iw_event_flag:0x%x\n",
        wl_iw_event_debug, flag);
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	uint32 status =  ntoh32(e->status);
	uint32 toto;
    uint32 reason = ntoh32_ua((void *)&e->reason);
	...
	case WLC_E_DISASSOC_IND:
		...
		wrqu.addr.sa_family = ARPHRD_ETHER;
		bzero(&extra, ETHER_ADDR_LEN);
        //w00176398 add for disconnect reason
        sprintf(wrqu.addr.sa_data + ETHER_ADDR_LEN + 1, "%d", reason);
        WL_TRACE(("disconnect reason111:%d\n", strtoul(wrqu.addr.sa_data + ETHER_ADDR_LEN + 1,NULL, 0)));
		break;
	case WLC_E_LINK:
	case WLC_E_NDIS_LINK:
		...
		g_ss_cache_ctrl.m_link_down = 1;
			...
			if (ap_cfg_running && !strncmp(dev->name, "wl0.1", 5)) {
				...
				//w00176398
				WIFI_STA_SPIN_LOCK(g_sta_flux.flux_spinlock);
				g_sta_flux.is_sta_connect = true;
				WIFI_STA_SPIN_UNLOCK(g_sta_flux.flux_spinlock);
				wifi_idle_timer_start(g_sta_flux.disconnect_time);
			}
#else
	...
	case WLC_E_SET_SSID:
	{
		if (status != WLC_E_STATUS_SUCCESS){
			...
			WL_TRACE(("%s: WLC_E_SET_SSID, connect to AP failed, restart AP.\n", __FUNCTION__));
