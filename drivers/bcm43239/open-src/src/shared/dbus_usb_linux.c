...
extern int dhd_set_hostwake(void *pat,bool bset);
...
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 22)) && defined(BCM43239_CONFIG_USB_SUSPEND))
...
static int dbus_usbos_resume(struct usb_interface *intf)
{
	...
	if (g_probe_info.intf->pm_usage_cnt.counter == 0) {
		g_probe_info.intf->pm_usage_cnt.counter = 1;
#else
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(2, 6, 32))
	g_probe_info.intf->pm_usage_cnt.counter = 0;
#else
