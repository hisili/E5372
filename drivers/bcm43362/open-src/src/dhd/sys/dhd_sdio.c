...
#if defined(BALONG_PLATFORM)
//=========
static uint32 g_st_total_packet_txglom = 0L;
static uint32 g_st_times_txglom = 0L;
static uint32 g_st_total_packet_rxglom = 0L;
static uint32 g_st_times_rxglom = 0L;

void dhd_glom_dump(void);
//=========
#endif
...
#ifdef GLOM_CODES
#define TX_GLOM_PKT_MIN_LEN 1000
#define TX_GLOM_PKT_MAX_LEN 1656
//w00176398 2011-12-01
//#define TX_GLOM_BUCKET_NUM  8
#define TX_GLOM_BUCKET_NUM  1
#define TX_GLOM_BUCKET_SIZE 1680
#define TX_GLOM_RECV_OFFSET 8
	void*		glom_pkt_buf;
	uint16 		glom_pkt_idx;
	uint16		glom_last_pkt_len;
	uint32		glom_pkt_swtag;

#define TX_GLOM_MAX_NUM	TX_GLOM_BUCKET_NUM
	void *		glom_skb[TX_GLOM_MAX_NUM+1];
#endif
...
const static char *bcm43362a0_fw_name[] = {
	"fw_bcm43362a0.bin",
	"fw_bcm43362a0_apsta.bin", //43362a0-roml.5.90.183.sdio-apsta-idauth-pktfilter-keepalive.bin
	"fw_bcm43362a0_p2p.bin",
	"fw_bcm43362a0_mfg.bin"	
};
const static char *bcm43362a2_fw_name[] = {
	"fw_bcm43362a2.bin",
	"fw_bcm43362a2_apsta.bin", //2011-12-20 43362a2-roml.5.90.183.sdio-apsta-idauth-pktfilter-keepalive.bin
...
#if defined(BALONG_PLATFORM)
#define BL_SKB_MAXLEN 1536

static int dhd_bcmsdh_recv_skbbuf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags,
	uint8 *buf, uint nbytes,
	void *pkt, bcmsdh_cmplt_fn_t complete, void *handle);
#endif
...
static int dhdsdio_glom_pkt_init(dhd_bus_t *bus, osl_t *osh);
...
{
#if defined(HW_OOB)
	DHD_INTR(("%s: enable HW_OOB!!\n", __FUNCTION__));
	bcmsdh_enable_hw_oob_intr(bus->sdh, enable);
#else
	sdpcmd_regs_t *regs = bus->regs;
	uint retries = 0;

	DHD_INTR(("%s: enable SW_OOB!!\n", __FUNCTION__));
...
static int
dhdsdio_txpkt(dhd_bus_t *bus, void *pkt, uint chan, bool free_pkt, bool last_pkt)
{
	int ret;
	osl_t *osh;
	uint8 *frame;
	...
	*(((uint16*)frame) + 1) = htol16(~len);
	...
	if(wifi_trace_pri_check(WIFI_TX_FRAME))
	{
		wifi_trace_add(WIFI_TX_FRAME,frame,len);
	}
#endif


#if defined(TXGLOM_CHAIN) && defined(BALONG_PLATFORM)
	if (len < TX_GLOM_PKT_MIN_LEN) {
#endif
	...
	do {
		ret = dhd_bcmsdh_send_buf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
		                          frame, len, pkt, NULL, NULL, last_pkt);
		bus->f2txdata++;
	...
	if ( ret == 1)
	{
		DHD_INFO((">>>enqueue-info : skb=0x%p, data=0x%x\n", pkt, ((uint32)PKTDATA(osh, pkt))));
		bus->tx_seq = (bus->tx_seq + 1) % SDPCM_SEQUENCE_WRAP;
		ret = 0;
	}
	else if (ret == 2) {
		PKTFREE(osh, bus->glom_pkt_buf, TRUE);
		bus->glom_pkt_buf = NULL;

		bus->tx_seq = (bus->tx_seq + 1) % SDPCM_SEQUENCE_WRAP;
		DHD_EVENT(("dhdsdio_txpkt: tx_max=%d tx_seq=%d glom_pkt_idx=%d\n", bus->tx_max, bus->tx_seq, bus->glom_pkt_idx));

		// for txglom statistic
		g_st_total_packet_txglom += bus->glom_pkt_idx;
		g_st_times_txglom += 1;
...
#if defined(BALONG_PLATFORM)
	int ifidx; 
    dhd_prot_ifidx_no_offset(bus->dhd, &ifidx, pkt);
    DHD_TIMER(("222%s ifidx:%d\n", __FUNCTION__,ifidx));
#endif
...
#endif
        DHD_TIMER(("%s ENTER111\n", __FUNCTION__));
#if defined(BALONG_PLATFORM)
        //dhd_prot_ifidx(bus->dhd, &ifidx, pkt);
        DHD_TIMER(("222%s: ret:%d ifidx:%d datalen:%d\n", __FUNCTION__,ret,ifidx,datalen));
		if (ret)
			bus->dhd->dstats[ifidx].tx_errors++;
		else
			bus->dhd->dstats[ifidx].tx_bytes += datalen;
#else
		if (ret)
			bus->dhd->tx_errors++;
		else
...
dhdsdio_sendfromq(dhd_bus_t *bus, uint maxframes)
{
	...
	for (cnt = 0; (cnt < maxframes) && DATAOK(bus); cnt++) {
        ...
        dhd_prot_ifidx(bus->dhd, &ifidx, pkt);
        DHD_TIMER(("111%s ifidx:%d\n", __FUNCTION__,ifidx));
        
		dhd_os_sdunlock_txq(bus->dhd);
		datalen = PKTLEN(bus->dhd->osh, pkt) - SDPCM_HDRLEN;

#if defined(BALONG_PLATFORM)
		if (pktq_empty(&bus->txq) == TRUE) {
			DHD_TXGLOM(("%s: bus->txq is empty\n", __FUNCTION__));
			lastpkt = TRUE;
		}
#endif

#ifndef SDTEST
		ret = dhdsdio_txpkt(bus, pkt, SDPCM_DATA_CHANNEL, TRUE, lastpkt);
#else
		ret = dhdsdio_txpkt(bus, pkt,
		        (bus->ext_loop ? SDPCM_TEST_CHANNEL : SDPCM_DATA_CHANNEL), TRUE, lastpkt);
#endif

#if defined(BALONG_PLATFORM)
        //dhd_prot_ifidx(bus->dhd, &ifidx, pkt);
        DHD_TIMER(("111%s: ret:%d ifidx:%d datalen:%d\n", __FUNCTION__,ret,ifidx,datalen));
		if (ret)
			bus->dhd->dstats[ifidx].tx_errors++;
		else
			bus->dhd->dstats[ifidx].tx_bytes += datalen;
...
dhd_bus_txctl(struct dhd_bus *bus, uchar *msg, uint msglen)
{
	...
	*(((uint16*)frame) + 1) = htol16(~msglen);
	...
	if (ret == -1) {
		...
		do {
			ret = dhd_bcmsdh_send_buf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
			                          frame, len, NULL, NULL, NULL, FALSE);
			ASSERT(ret != BCME_PENDING);
...
//w00176398 add for open debug
#if defined(DHD_DEBUG)
	DHD_ERROR(("%s=======enabled inchip-debug=========\n", __FUNCTION__));
	if(dhd_serialconsole(bus, TRUE, TRUE, &err) < 0)
	{
		DHD_ERROR(("%s: call dhd_serialconsole error(%d)\n",__FUNCTION__,err));
	}
#endif
...
dhdsdio_rxglom(dhd_bus_t *bus, uint8 rxseq)
{
	...
	DHD_GLOM(("dhdsdio_rxglom: start: glomd %p glom %p\n", bus->glomd, bus->glom));
...
#if defined(BALONG_PLATFORM)
				// to split as two skb for workaround skb length issue;
				ppad = NULL;

				// for last frame, once the sublen is bigger than 1532(BALONG required), 
				//  we have to allocate a new skb for the padding;
				if (sublen > BL_SKB_MAXLEN) {
					if ((ppad = PKTGET(osh, sublen - BL_SKB_MAXLEN, FALSE))== NULL) {
						DHD_ERROR(("%s: PKTGET failed, num %d len %d\n",
						           __FUNCTION__, num+1, sublen - BL_SKB_MAXLEN));
						break;
					}
					ASSERT(!PKTLINK(ppad));
					DHD_GLOM(("%s: [%d]sublen=%d\n", __FUNCTION__, num, sublen));
					sublen = BL_SKB_MAXLEN;
					DHD_GLOM(("%s: ppadskb=%p, sublen=%d\n", __FUNCTION__, ppad, sublen));
				}

				g_st_total_packet_rxglom += num;
				g_st_times_rxglom += 1;
#endif
			}
...
#if defined(BALONG_PLATFORM)
			if ((pnext = PKTGET(osh, sublen - firstread, FALSE)) == NULL) {
#else
...
#if defined(BALONG_PLATFORM)
			/* Leave room for Header, Balong will sure it has enough headroom for save 32bytes */
			PKTPUSH(osh, pnext, firstread);
			DHD_GLOM(("%s: 2[%d] skblen=%d, sublen=%d\n", __FUNCTION__, num, PKTLEN(osh, pnext), sublen));
#endif
			if (!pfirst) {
				ASSERT(!plast);
				pfirst = plast = pnext;
	...
	if (bus->glom) {
		...
		if (usechain) {
#if defined(BALONG_PLATFORM)
			DHD_GLOM(("%s: call dhd_bcmsdh_recv_skbbuf for geting data to skb-chain\n", __FUNCTION__));
			DHD_GLOM(("%s: buf=%p buf_len=%d skb=%p\n", __FUNCTION__, PKTDATA(osh, pfirst), dlen, pfirst));

			errcode = dhd_bcmsdh_recv_skbbuf(bus,
		...
		if (ppad != NULL)	{
			// free pad's skb
			PKTSETNEXT(osh, plast, NULL);
			DHD_GLOM(("Freed pad skb=0x%p, it is for padding stuff\n", ppad));
			PKTFREE(osh, ppad, FALSE);
			ppad = NULL;
		}
#endif
...
#ifdef BALONG_WIFI_TRACE_THREAD
        if(wifi_trace_pri_check(WIFI_RXSUP_FRAME))
        {
            wifi_trace_add(WIFI_RXSUP_FRAME,PKTDATA(osh, pfirst),MIN(PKTLEN(osh, pfirst), 48));
        }
#endif
		...
		if ((uint8)(txmax - bus->tx_seq) > 0x40) {
			DHD_ERROR(("%s: got unlikely tx max %d with tx_seq %d, bus->tx_max %d\n",
...
#ifdef BALONG_WIFI_TRACE_THREAD
            if(wifi_trace_pri_check(WIFI_RXSUBDATA_FRAME))
            {
                wifi_trace_add(WIFI_RXSUBDATA_FRAME,dptr,dlen);
            }
#endif
	...
	for (rxseq = bus->rx_seq, rxleft = maxframes;
		...
		if (dhd_readahead && bus->nextlen) {
			...
			if (!(pkt = PKTGET(osh, rdlen + DHD_SDALIGN, FALSE))) {
				...
				DHD_DATA(("%s: failed at allocated a skb-buffer\n", __FUNCTION__));
				...
				/* Read the entire frame */
#if defined(BALONG_PLATFORM)
				DHD_DATA(("%s: [1]calling skbbuf\n", __FUNCTION__));
				DHD_INFO(("%s: buf=%p buf_len=%d skb=%p\n", __FUNCTION__, rxbuf, rdlen, pkt));
				sdret = dhd_bcmsdh_recv_skbbuf(bus, bcmsdh_cur_sbwad(sdh),
				                            SDIO_FUNC_2,
				                            F2SYNC, rxbuf, rdlen,
				                            pkt, NULL, NULL);
#else
...
#ifdef BALONG_WIFI_TRACE_THREAD
            if(wifi_trace_pri_check(WIFI_RX_FRAME))
            {
                wifi_trace_add(WIFI_RX_FRAME,rxbuf,len);
            }
#endif
		...
		dhd_os_sdlock_rxq(bus->dhd);
#if defined(BALONG_PLATFORM)
		// added by ghentlai@broadcom.com to minimum skb size due to the limitiations of Balong memory
		if (rdlen > 0)  {
			if (!(pkt = PKTGET(osh, rdlen, FALSE))) {
				/* Give up on data, request rtx of events */
		...
		// added by ghentlai@broadcom.com, if the remaining frame data length is bigger than 0, 
		//   we will build a buffer for getting it.
		if (rdlen > 0 ) { 
#if defined(BALONG_PLATFORM)
			DHD_DATA(("%s: [2]calling skbbuf\n", __FUNCTION__));
			DHD_INFO(("%s: buf=0x%p buf_len=%d skb=0x%p\n", __FUNCTION__, PKTDATA(osh, pkt), rdlen, pkt));
			sdret = dhd_bcmsdh_recv_skbbuf(bus, bcmsdh_cur_sbwad(sdh), SDIO_FUNC_2, F2SYNC,
			...
			DHD_TRACE(("%s: the remaining data is empty, skip the read action\n", __FUNCTION__));
		}
		...
		if (DHD_BYTES_ON() && DHD_DATA_ON()) {
            ...
            if(wifi_trace_pri_check(WIFI_RX_FRAME))
            {
                wifi_trace_add(WIFI_RX_FRAME,PKTDATA(osh, pkt),len);
            }
#endif
...
dhdsdio_dpc(dhd_bus_t *bus)
{
	...
	intstatus |= newstatus;
    DHD_INFO(("%s: intstatus new 0x%x,old=0x%x\n",__FUNCTION__,intstatus,newstatus));
	bus->intstatus = 0;
...
#ifdef BALONG_WIFI_TRACE_THREAD
			if(wifi_trace_pri_check(WIFI_TXGEN_FRAME))
			{
				wifi_trace_add(WIFI_TXGEN_FRAME,,data,PKTLEN(osh, pkt) - SDPCM_HDRLEN);
			}
#endif
...
dhd_bus_watchdog(dhd_pub_t *dhdp)
{
	...
	if ((bus->idletime > 0) && (bus->clkstate == CLK_AVAIL)) {
		...
		DHD_TIMER(("%s: bus->idletime=%d, bus->idlecount=%d\n", __FUNCTION__, bus->idletime, bus->idlecount));
	}
...
#ifdef GLOM_CODES
	if (BCME_OK != dhdsdio_glom_pkt_init(bus, osh)) {
		DHD_ERROR(("%s: failed at Txglom feature initialization\n", __FUNCTION__));
		goto fail;
	}
	else {
		DHD_EVENT(("%s: Enabled txglomming\n", __FUNCTION__));
	}
#endif
...
#ifdef GLOM_CODES
static int
dhdsdio_glom_pkt_init(dhd_bus_t *bus, osl_t *osh)
{
#if !defined(TXGLOM_CHAIN)	
	uint16 glom_pkt_len;
	uint8 *frame;
#else
	int	i;
...
#if !defined(TXGLOM_CHAIN)
	DHD_EVENT(("%s: =========enable rxglom(chip) with static buffer============\n", __FUNCTION__));
	glom_pkt_len = TX_GLOM_BUCKET_SIZE * TX_GLOM_BUCKET_NUM + DHD_SDALIGN;
	if (!(bus->glom_pkt_buf = PKTGET(osh, glom_pkt_len, TRUE))) {
		DHD_ERROR(("%s: PKTGET failed: len %d\n", __FUNCTION__, glom_pkt_len));
		return BCME_NOMEM;
        }

	PKTALIGN(osh, bus->glom_pkt_buf, PKTLEN(osh, bus->glom_pkt_buf), DHD_SDALIGN);

	frame = (uint8*)PKTDATA(osh, bus->glom_pkt_buf);
	bzero(frame, SDPCM_HDRLEN);
#else // for zero-copy
	bus->glom_pkt_buf = NULL;
	DHD_EVENT(("%s: =========enable rxglom(chip) txchain============\n", __FUNCTION__));

	for(i=0; i<TX_GLOM_MAX_NUM+1;i++)
		bus->glom_skb[i] = NULL;
#endif

	/* build software tag: channel, sequence number, data offset */
	bus->glom_pkt_swtag = ((SDPCM_GLOM_CHANNEL << SDPCM_CHANNEL_SHIFT) & SDPCM_CHANNEL_MASK) |
...
void dhd_bus_select_firmware_name_by_chip(struct dhd_bus * bus, char *dst, char * src)
{
	int fw_type = strstr(src, "_apsta") ? FW_TYPE_APSTA : 
					strstr(src, "_p2p") ? FW_TYPE_P2P : 
						strstr(src, "_mfg") ? FW_TYPE_MFG : FW_TYPE_STA;
	int i;
	...
	case BCM43362_CHIP_ID:
		...
		break;
	default:
		DHD_ERROR(("Unknow chipid=0x%x chiprev=0x%x\n", bus->sih->chip, bus->sih->chiprev));
		strcpy(dst, src);
		break;
	}
}
...
dhdsdio_download_code_array(struct dhd_bus *bus)
{
	...
	unsigned char *code_array = NULL;
	unsigned char *code_align = NULL;

#if 1
	unsigned char *ularray;
	unsigned char *ularray_align;
#endif

	DHD_INFO(("%s: download embedded firmware...\n", __FUNCTION__));

	DHD_INFO(("\n\n\t\t\t\t\t\tstarting to download firmware for array:0x%08x\n\n", dlarray));

	code_array = MALLOC_EXT(bus->dhd->osh, MEMBLOCK+MEMBLOCK);
	if (code_array == NULL)
	{
		DHD_ERROR(("failed at allocate memory for code_array\n"));
		goto err;
	}
	memset(code_array, 0xFF, MEMBLOCK+MEMBLOCK);
	code_align = (unsigned char *)((((int)code_array + MEMBLOCK - 1)) & (~(MEMBLOCK - 1))); // align with MEMBLOCK
	DHD_INFO(("====>code_array=%x code_align=%x\n", code_array, code_align));

	/* Download image */
	while ((offset + MEMBLOCK) < sizeof(dlarray)) {
		memcpy(code_align, dlarray + offset, MEMBLOCK);

		bcmerror = dhdsdio_membytes(bus, TRUE, offset, code_align, MEMBLOCK);
		if (bcmerror) {
	...
	if (offset < sizeof(dlarray)) {
		memcpy(code_align, dlarray + offset, sizeof(dlarray) - offset);

		bcmerror = dhdsdio_membytes(bus, TRUE, offset, code_align, sizeof(dlarray) - offset);
		if (bcmerror) {
			DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
	...
	DHD_INFO(("\n\n\t\t\t\t\t\tbeginning to verify the firmware is download correctly\n\n"));
	...
	int corrupted_blockn = 0, n = 0;
	char * blockptr = NULL;

        ularray = MALLOC_EXT(bus->dhd->osh, MEMBLOCK+MEMBLOCK);
        if (ularray == NULL)
        {
            DHD_ERROR(("failed at allocate memory for ularray\n"));
            goto err;
        }
	ularray_align = (unsigned char *)((((int)ularray + MEMBLOCK - 1)) & (~(MEMBLOCK - 1))); // align with 2k
        DHD_INFO(("====>ularray=%x ularray_align=%x\n", ularray, ularray_align));

        /* Upload image to verify downloaded contents. */
        offset = 0;
        memset(ularray, 0xaa, MEMBLOCK);
        blockptr = (char*)dlarray;
        while ((offset + MEMBLOCK) < sizeof(dlarray)) {
            bcmerror = dhdsdio_membytes(bus, FALSE, offset, ularray_align, MEMBLOCK);
            if (bcmerror) {
                DHD_ERROR(("%s: error %d on reading %d membytes at 0x%08x\n",
                    __FUNCTION__, bcmerror, MEMBLOCK, offset));
                goto err;
            ...
            if (memcmp(blockptr, ularray_align, MEMBLOCK)) {
                DHD_ERROR(("%d: [offset=0x%08x]Downloaded image is corrupted.\n", n, offset));
                //ASSERT(0);
                //goto err;

                #ifdef BWL_MOBCOM_DBGPRINTF
                //DHD_ERROR(("[%d] dlarray:0x%x offset:0x%x blockptr:0x%x, failed to compare\n", n, dlarray, offset, blockptr));
                prhex("FW ARRAY", (char*)blockptr, MEMBLOCK);
                prhex("FW DEV", (char*)ularray_align, MEMBLOCK);
                osl_delay(1000*1000);
                #endif
                ...
                DHD_EVENT(("%d: [offset=0x%08x]Download, Upload and compare succeeded.\n", n, offset));
                //printf_hexDump("From-chip", (char*)ularray_align, MEMBLOCK);
            }

            blockptr += MEMBLOCK;
            offset   += MEMBLOCK;
            n++;
            //osl_delay(1000 * 1000); // 1000ms
        }

        if (offset < sizeof(dlarray)) {
            bcmerror = dhdsdio_membytes(bus, FALSE, offset, ularray_align, sizeof(dlarray) - offset);
            if (bcmerror) {
                DHD_ERROR(("%s: error %d on reading %d membytes at 0x%08x\n",
                    __FUNCTION__, bcmerror, sizeof(dlarray) - offset, offset));
                goto err;
            ...
            if (memcmp(blockptr, ularray_align, sizeof(dlarray) - offset)) {
                DHD_ERROR(("%d: [offset=0x%08x]Downloaded image is corrupted.\n", n, offset));
                //ASSERT(0);
                //goto err;
                #ifdef BWL_MOBCOM_DBGPRINTF
                //DHD_ERROR(("[the last block] dlarray:0x%x offset:0x%x blockptr:0x%x, failed to compare\n", dlarray, offset, blockptr));
                prhex("FW ARRAY", (char*)blockptr, sizeof(dlarray) - offset);
                prhex("FW DEV", (char*)ularray_align, sizeof(dlarray) - offset);
                #endif
                ...
                DHD_EVENT(("%d: [offset=0x%08x]Download, Upload and compare succeeded.\n", n, offset));
                //prhex("From-chip", (char*)ularray_align, sizeof(dlarray) - offset);
            }

        }

        if (corrupted_blockn == 0)
            DHD_INFO(("\n\n\t\t\t\t\t\tDownload, Upload and compare succeeded.\n\n"));
...
#ifdef DHD_DEBUG
    if (ularray) MFREE_EXT(bus->dhd->osh, ularray, MEMBLOCK+MEMBLOCK);
#endif
    if (code_array) MFREE_EXT(bus->dhd->osh, code_array, MEMBLOCK+MEMBLOCK);
...
dhdsdio_download_code_file(struct dhd_bus *bus, char *pfw_path)
{
    ...
    memptr = memblock = (uint8*)MALLOC(bus->dhd->osh, MEMBLOCK + DHD_SDALIGN);
	if (memblock == NULL) {
	...
	/* Download image */
    while (0 != (len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {
#if 1
		...
		timeuse_usec = (1000000*(t_end.tv_sec - t_start.tv_sec) + t_end.tv_usec) - t_start.tv_usec;
		printk("%s ---------count=%d  t_end.tv_sec=%d, t_end.tv_usec=%d\n", __func__, count, (int)t_end.tv_sec, (int)t_end.tv_usec);
		printk("qijiwen use write time: %d usec\n", timeuse_usec);


		printk("start -------------------------- read  len=%d\n", len);
		do_gettimeofday(&t_start);
		for (count = 0; count < 1000; count++)
		{
			bcmerror = dhdsdio_membytes(bus, FALSE, offset, memptr, len);
			if (bcmerror) {
				DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
					__FUNCTION__, bcmerror, MEMBLOCK, offset));
				goto err;
		...
		timeuse_usec = (1000000*(t_end.tv_sec - t_start.tv_sec) + t_end.tv_usec) - t_start.tv_usec;
		printk("%s ---------count=%d  t_end.tv_sec=%d, t_end.tv_usec=%d\n", __func__, count, (int)t_end.tv_sec, (int)t_end.tv_usec);
		printk("qijiwen use read time: %d usec\n", timeuse_usec);
#endif
...
#if 0 && defined(DHD_DEBUG)
static int
dhdsdio_download_code_dump(struct dhd_bus *bus, char *fw_path)
{
    int bcmerror = -1;
    int offset = 0;
    uint len;
    void *image = NULL;
    uint8 *memblock = NULL, *memptr;
    uint8 *rb_memblock = NULL, *rb_memptr;

    DHD_INFO(("%s: readback firmware for check\n", __FUNCTION__));
    ...
    ////
    rb_memptr = rb_memblock = (uint8*)MALLOC(bus->dhd->osh, MEMBLOCK + DHD_SDALIGN);
    if (rb_memblock == NULL) {
        DHD_ERROR(("%s: Failed to allocate memory %d bytes\n", __FUNCTION__, MEMBLOCK));
        goto err;
    }
    if ((uint32)(uintptr)rb_memblock % DHD_SDALIGN)
        rb_memptr += (DHD_SDALIGN - ((uint32)(uintptr)rb_memblock % DHD_SDALIGN));
    ////

    /* Upload image */
    while (0 != (len = dhd_os_get_image_block((char*)memptr, MEMBLOCK, image))) {
        bcmerror = dhdsdio_membytes(bus, FALSE, offset, rb_memptr, len);
        if (bcmerror) {
            DHD_ERROR(("%s: error %d on writing %d membytes at 0x%08x\n",
                    __FUNCTION__, bcmerror, MEMBLOCK, offset));
            goto err;
            ...
            if (0 != memcmp(rb_memptr, memptr, MEMBLOCK)) {
                DHD_ERROR(("%s: Fw-upload is corrupted at offset=0x%x\n", __FUNCTION__, offset));
            }
#if 0 && defined(DHD_DEBUG)
            else {
                DHD_EVENT(("%s: Fw-upload is correctly at offset=0x%x\n", __FUNCTION__, offset));
                prhex("FW-upload", (char*)memptr, len);
            }
#endif
            osl_delay(100* 1000);
        }
...
err:
    ...
    if (rb_memblock)
        MFREE(bus->dhd->osh, rb_memblock, MEMBLOCK + DHD_SDALIGN);
...
dhdsdio_download_nvram(struct dhd_bus *bus)
{
    ...
    memblock = (char*)MALLOC(bus->dhd->osh, MAX_NVRAMBUF_SIZE);
	if (memblock == NULL) {
		DHD_ERROR(("%s: Failed to allocate memory %d bytes\n",
...
_dhdsdio_download_firmware(struct dhd_bus *bus)
{
	...
	if (embed) {
		if (dhdsdio_download_code_array(bus)) {
            ...
            DHD_INFO(("%s: dongle image array download success\n", __FUNCTION__));
			dlok = TRUE;
		}
	}
#else
#if 0 && defined(DHD_DEBUG)
	dhdsdio_download_code_dump(bus, bus->fw_path);
#endif
#endif
	if (!dlok) {
...
//w00176398 add for testing
int gBufThred = 2;
void SetBufThred(uint16 value)
{
    printk("old gBufThred:%d new gBufThred:%d \n", gBufThred, value);
    gBufThred = value;
}

static int
dhd_bcmsdh_send_buf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags, uint8 *buf, uint nbytes,
	void *pkt, bcmsdh_cmplt_fn_t complete, void *handle, bool lastpkt)
{
#ifdef GLOM_CODES
#define DHD_TXGLOM_PADDING 4
	osl_t 			*osh;
	int 				pkt_len = 0;
	unsigned char 	*pData;
	int				tx_q_len; 
	...
	if (buf != NULL && nbytes != 0 && pkt == NULL) {
		DHD_INFO(("%s: This is a control packet, send it directly\n", __FUNCTION__));

#ifdef BALONG_WIFI_TRACE_THREAD
        if(wifi_trace_pri_check(WIFI_CTL_FRAME))
        {
            wifi_trace_add(WIFI_CTL_FRAME,buf,nbytes);
        }        
#endif
	...
	DHD_TXGLOM(("%s: downlink glom_pkt_idx=%d tx_max=%d, tx_seq=%d, tx_max-tx_seq=%ld, rx_seq=%d\n", 
				__FUNCTION__, bus->glom_pkt_idx, 
				bus->tx_max, bus->tx_seq, abs(bus->tx_max-bus->tx_seq),
				bus->rx_seq));

	tx_q_len = abs(bus->tx_max-bus->tx_seq);

	// lastpkt == TRUE is meaning this is the last packet from sendQ, dhd should stop txgloom and send packets directly
	//  tx_q_len is the queue size of firmware's rx buffer, to sure it has room to save the txgloom
	if ((pkt != NULL && lastpkt == FALSE) ||
		tx_q_len > (bus->glom_pkt_idx+gBufThred))
	{

		pkt_len = PKTLEN(osh, pkt);

		ASSERT( pkt_len <= TX_GLOM_PKT_MAX_LEN);

        	osh = bus->dhd->osh;
#if !defined(TXGLOM_CHAIN)
		pData = PKTDATA(osh, bus->glom_pkt_buf);
#endif
		DHD_TRACE(("%s: downlink nbytes=%d, pkt_len=%d, tx_q_len=%d\n", 
					__FUNCTION__, nbytes, pkt_len, tx_q_len));

		if ( (pkt_len >= TX_GLOM_PKT_MIN_LEN) && DATAOK(bus) && (bus->glom_pkt_idx < TX_GLOM_BUCKET_NUM) ) {

			if (bus->glom_pkt_idx == 0) {
				uint swheader;
#if defined(TXGLOM_CHAIN)
				uint offset = 0;
				uint8 * frame;
				...
				bucket_size = TX_GLOM_BUCKET_SIZE - TX_GLOM_RECV_OFFSET - SDPCM_HDRLEN;

#if !defined(TXGLOM_CHAIN)
				/* first entry has fewer space */
				p = pData + SDPCM_HDRLEN; 

				/* software tag: channel, sequence number, data offset */
 				swheader = bus->glom_pkt_swtag | bus->tx_seq;
        			htol32_ua_store(swheader, pData + SDPCM_FRAMETAG_LEN);
        			htol32_ua_store(0, pData + SDPCM_FRAMETAG_LEN + sizeof(swheader));

				if ( PKTLEN(osh, pkt) > bucket_size ){
					DHD_ERROR(("%s: #%d glom packet is overflow. %d/%d\n", __FUNCTION__, bus->glom_pkt_idx, PKTLEN(osh, pkt), bucket_size));
				}

				memcpy( p, buf, MIN(nbytes, bucket_size));
				bus->glom_pkt_idx++;	

				DHD_TXGLOM(("%s: [%d] first glom tx seq = %d len=%d\n", __FUNCTION__, bus->glom_pkt_idx, bus->tx_seq, MIN(nbytes, bucket_size)));
				prhex("first-glom", pData, 32);

#else
				ASSERT(bus->glom_pkt_buf == NULL);

				/* Check to see if the resever room is enough for any header */
				if ((uint32)PKTHEADROOM(osh, pkt) < (32)) {
					DHD_ERROR(("%s: insufficient headroom %d for adding Txglomming descriptor\n",
			          			__FUNCTION__, (int)PKTHEADROOM(osh, pkt)));
				}

				DHD_TRACE(("fq-txglom1, skb=0x%p, data=0x%x, len=%d\n", pkt, ((uint32)PKTDATA(osh, pkt)), PKTLEN(osh, pkt)));
				//prhex("before add HW&SW", PKTDATA(osh, pkt), 32);

				PKTPUSH(osh, pkt, SDPCM_HDRLEN);
				DHD_TRACE(("fq-txglom2, skb=0x%p, data=0x%x, len=%d\n", pkt, ((uint32)PKTDATA(osh, pkt)), PKTLEN(osh, pkt)));
				frame = (uint8*)PKTDATA(osh, pkt);

				pad1 = SDPCM_HDRLEN;
				/* software tag: channel, sequence number, data offset */
				/* build software tag: channel, sequence number, data offset */
				//swheader = ((SDPCM_GLOM_CHANNEL << SDPCM_CHANNEL_SHIFT) & SDPCM_CHANNEL_MASK)
				//       | bus->tx_seq | ((pad1 << SDPCM_DOFFSET_SHIFT) & SDPCM_DOFFSET_MASK);
 				swheader = bus->glom_pkt_swtag | bus->tx_seq;

				//prhex("sw_header", (uchar*)&swheader, 4);
				offset = 0;
				// set hardware_header default as 0xCA
				htol32_ua_store(0xCACACACA, PKTDATA(osh, pkt) + offset);
				offset += SDPCM_FRAMETAG_LEN;
				// set software_header
				htol32_ua_store(swheader, PKTDATA(osh, pkt) + offset);
				htol32_ua_store(0, (PKTDATA(osh, pkt) + offset + sizeof(swheader)));

				if ( PKTLEN(osh, pkt) > bucket_size) {
					DHD_ERROR(("#%d glom packet is overflow. %d/%d\n", bus->glom_pkt_idx, PKTLEN(osh, pkt), bucket_size));
				}

				PKTSETLEN(osh, pkt, TX_GLOM_BUCKET_SIZE-TX_GLOM_RECV_OFFSET);
				DHD_TRACE(("fq-txglom4, data=0x%x, len=%d\n", ((uint32)PKTDATA(osh, pkt)), PKTLEN(osh, pkt)));

				bus->glom_skb[bus->glom_pkt_idx++] = pkt;
				bus->glom_pkt_buf = pkt;
				PKTSETNEXT(osh, pkt, NULL);

				// clean the all skb-pointer of txglomming-list
				for (i=1; i<TX_GLOM_MAX_NUM;i++) {
					bus->glom_skb[i] = NULL;
				}

#if 0 && defined(DHD_DEBUG)
				for (i=0; i<TX_GLOM_MAX_NUM;i++) {
					if (bus->glom_skb[i] != NULL)
						DHD_TXGLOM(("fq-txglom, data=0x%x, len=%4d, next=0x%x\n", ((uint32)PKTDATA(osh, bus->glom_skb[i])), PKTLEN(osh, bus->glom_skb[i]), ((uint32)PKTNEXT(osh, bus->glom_skb[i]))));
					else
						DHD_TXGLOM(("fq-txglom, data=NULL\n"));
				}
#endif
#endif
...
#if !defined(TXGLOM_CHAIN)
				p = pData + (TX_GLOM_BUCKET_SIZE * bus->glom_pkt_idx) - TX_GLOM_RECV_OFFSET;
				bucket_size = TX_GLOM_BUCKET_SIZE;
				...
				memcpy( p, buf, MIN(nbytes, bucket_size));
				DHD_TXGLOM(("%s: [%d] offset = %d, len=%d\n", __FUNCTION__, bus->glom_pkt_idx, (TX_GLOM_BUCKET_SIZE * bus->glom_pkt_idx) - TX_GLOM_RECV_OFFSET, MIN(nbytes, bucket_size)));
				bus->glom_pkt_idx++;	
				prhex("tx-glom", pData, 32);				
#else

				if (((bus->glom_pkt_idx+1)>=TX_GLOM_MAX_NUM) 
					|| ((bus->glom_pkt_idx+1) >= abs(bus->tx_max-bus->tx_seq))) {
				}
				else {
					...
					// add padding
					PKTSETLEN(osh, pkt, TX_GLOM_BUCKET_SIZE);
					ASSERT(PKTNEXT(osh, pkt) == NULL);
					PKTSETNEXT(osh, bus->glom_skb[bus->glom_pkt_idx-1], pkt);
					PKTSETNEXT(osh, pkt, NULL);
					bus->glom_skb[bus->glom_pkt_idx++] = pkt;
					...
					for (i=0; i<TX_GLOM_MAX_NUM;i++) {
						if (bus->glom_skb[i] != NULL)
							DHD_TXGLOM(("mq-txglom[%d], data=0x%x, len=%4d, next=0x%x\n", i, ((uint32)PKTDATA(osh, bus->glom_skb[i])), PKTLEN(osh, bus->glom_skb[i]), ((uint32)PKTNEXT(osh, bus->glom_skb[i]))));
						else
							DHD_TXGLOM(("mq-txglom[%d], data=NULL\n", i));
					}
#endif
	...
	if ( bus->glom_pkt_idx ) {
		...
		/* set glom packet length */
		len = TX_GLOM_BUCKET_SIZE*bus->glom_pkt_idx - TX_GLOM_RECV_OFFSET + ((pkt_len)?pkt_len:nbytes);
		PKTSETLEN(osh, bus->glom_pkt_buf, len);

		DHD_TXGLOM(("%s: nbytes=%d, pkt_len=%d, glom pkt_len = %d\n", __FUNCTION__, nbytes, pkt_len, len));

        	/* Hardware tag: 2 byte len followed by 2 byte ~len check (all LE) */
       	*(uint16*)pData = htol16(len);
        	*(((uint16*)pData) + 1) = htol16(~len);

		/* Don't do copy if the frame is flush signal */
		if ( nbytes > 0 ) {
			p = pData + (TX_GLOM_BUCKET_SIZE * bus->glom_pkt_idx) - TX_GLOM_RECV_OFFSET;
			memcpy( p, buf, MIN( nbytes, TX_GLOM_BUCKET_SIZE));
			DHD_TXGLOM(("%s: [%d] offset = %d len=%d\n", __FUNCTION__, bus->glom_pkt_idx, (TX_GLOM_BUCKET_SIZE * bus->glom_pkt_idx) - TX_GLOM_RECV_OFFSET, MIN( nbytes, TX_GLOM_BUCKET_SIZE)));
			prhex("the-last-txglom", buf, 32);
			bus->glom_pkt_idx++;
		} else 
			DHD_GLOM(("%s: got a flush signal\n", __FUNCTION__));
		...
		if (bus->roundup && bus->blocksize && (len > bus->blocksize)) {
			...
			if ((pad2 <= bus->roundup) && (pad2 < bus->blocksize))
#ifdef NOTUSED
				if (pad2 <= PKTTAILROOM(osh, bus->glom_pkt_buf))
#endif /* NOTUSED */
					len += pad2;
		} else if (len % DHD_SDALIGN) {
			len += DHD_SDALIGN - (len % DHD_SDALIGN);
		...
		if (forcealign && (len & (ALIGNMENT - 1))) { 
#ifdef NOTUSED 
			if (PKTTAILROOM(osh, bus->glom_pkt_buf))
#endif
				len = ROUNDUP(len, ALIGNMENT);
#ifdef NOTUSED
			else  
		...
		DHD_GLOM(("%s: sending %d glom pkt, len=%d\n", __FUNCTION__, bus->glom_pkt_idx, len));
		prhex("tx-glomming", pData, 64);

		/* send glom packets rigtaway */
		ret = bcmsdh_send_buf(bus->sdh, addr, fn, flags, pData, len, bus->glom_pkt_buf, complete, handle);
		...
		/* set glom packet length */
		total_len = TX_GLOM_BUCKET_SIZE*bus->glom_pkt_idx - TX_GLOM_RECV_OFFSET + PKTLEN(osh, pkt);

		DHD_TRACE(("sq-txglom nbytes=%d, pkt_len=%d, glom pkt_len = %d\n", nbytes, pkt_len, total_len));

        /* Hardware tag: 2 byte len followed by 2 byte ~len check (all LE) */	
		*(uint16*)pData = htol16(total_len);
		*(((uint16*)pData) + 1) = htol16(~total_len);

		//prhex("adding hardware_header", PKTDATA(osh, bus->glom_pkt_buf), 64);
		...
		if ( nbytes > 0 ) {
			...
			DHD_TXGLOM(("%s: got a flush signal\n", __FUNCTION__));
		}

		//calc the total lenth of txglomming skb-link
		for (i=0; i<TX_GLOM_MAX_NUM;i++) {
			if (bus->glom_skb[i] != NULL) {
				len += PKTLEN(osh, bus->glom_skb[i]);
#if 0 && defined(DHD_DEBUG)
				DHD_TXGLOM(("sq-txglom[%d], skb=0x%x, data=0x%x, len=%4d, next=0x%x\n", i, ((uint32)bus->glom_skb[i]), ((uint32)PKTDATA(osh, bus->glom_skb[i])), PKTLEN(osh, bus->glom_skb[i]), ((uint32)PKTNEXT(osh, bus->glom_skb[i]))));
				prhex("sq-txglom", PKTDATA(osh, bus->glom_skb[i]), PKTLEN(osh, pkt)>64?64:PKTLEN(osh, bus->glom_skb[i]));
			}
			else
				DHD_TXGLOM(("sq-txglom[%d], data=NULL\n", i));
#else
			}
            else
                break;
		...
		DHD_TXGLOM(("!!!!!! : total_len=%d, act_len=%d\n", total_len, len));
        ASSERT(total_len == len);

		alignment_size = total_len;
		/* Raise len to next SDIO block to eliminate tail command */
		if (bus->roundup && bus->blocksize && (alignment_size > bus->blocksize)) {
			uint16 pad2 = bus->blocksize - (alignment_size % bus->blocksize);
			if ((pad2 <= bus->roundup) && (pad2 < bus->blocksize))
#ifdef NOTUSED
				if (pad2 <= PKTTAILROOM(osh, pkt))
#endif /* NOTUSED */
					alignment_size += pad2;
		} else if (alignment_size % DHD_SDALIGN) {
			alignment_size += DHD_SDALIGN - (alignment_size % DHD_SDALIGN);
		}
                
		/* Some controllers have trouble with odd bytes -- round to even */
		if (forcealign && (alignment_size & (ALIGNMENT - 1))) { 
#ifdef NOTUSED 
			if (PKTTAILROOM(osh, pkt))
#endif          
				alignment_size = ROUNDUP(alignment_size, ALIGNMENT);
#ifdef NOTUSED  
	        else  
				DHD_ERROR(("%s: sending unrounded %d-byte packet\n", __FUNCTION__, alignment_size));
#endif
	       }

		lastpkt_len = alignment_size -TX_GLOM_BUCKET_SIZE*(bus->glom_pkt_idx-1)+TX_GLOM_RECV_OFFSET;
		DHD_TXGLOM(("%s: glom pkt, lastpkt_len=%d, act_len=%d\n", __FUNCTION__, lastpkt_len, PKTLEN(osh, pkt)));
		DHD_TXGLOM(("%s: send %d glom pkt, len=%d\n", __FUNCTION__, bus->glom_pkt_idx, alignment_size));
		DHD_TXGLOM(("%s: tailroom=%d\n", __FUNCTION__, PKTTAILROOM(osh, pkt)));

		if (lastpkt_len < PKTLEN(osh, pkt)) {
			//  if the length of lastpkt_len is less than PKTLEN,
			PKTSETLEN(osh, pkt, lastpkt_len);
			DHD_TXGLOM(("sq-txglom[tail] : update skb=0x%p, data=0x%x len=%d\n", pkt, ((uint32)PKTDATA(osh, pkt)), PKTLEN(osh, pkt)));
		}
		else if ( PKTTAILROOM(osh, pkt) >0 && (lastpkt_len-PKTLEN(osh, pkt)) <= PKTTAILROOM(osh, pkt)) {
			// if the length of laskpkt_len frame is bigger than tailroom, we have to allocate a new skb for adding pad.
			PKTSETLEN(osh, pkt, lastpkt_len);
			DHD_TXGLOM(("sq-txglom[tail] : update skb=0x%p, data=0x%x len=%d\n", pkt, ((uint32)PKTDATA(osh, pkt)), PKTLEN(osh, pkt)));
		}
		else {
			...
			len_u = PKTLEN(osh, pkt);
			len_u += 3;
			len_u &= 0xFFFFFFFC;
			PKTSETLEN(osh, pkt, len_u);

			newpkt = PKTGET(osh, lastpkt_len-PKTLEN(osh, pkt), TRUE);
			ASSERT(newpkt != NULL);
			DHD_TXGLOM(("%s: allocate a new skb buffer(%d=0x%x) for F2 blocksize alignment\n", 
						__FUNCTION__, bus->glom_pkt_idx, (uint32)newpkt));
			PKTSETNEXT(osh, pkt, newpkt);
			PKTSETNEXT(osh, newpkt, NULL);
			bus->glom_skb[bus->glom_pkt_idx++] = newpkt;
		}

		/* send glom packets rigtaway */
		ret = bcmsdh_send_skbbuf(bus->sdh, addr, fn, flags, (uint8*)PKTDATA(osh, bus->glom_pkt_buf), alignment_size, bus->glom_pkt_buf, complete, handle);

#ifdef GLOM_WORK_QUEUE
		/* schedule flush task, 10 ms */
		schedule_delayed_work(&(flush_glom_pkts.n), HZ);
#endif

#endif
		return 2;
	} else if ( nbytes == 0 ) {
		DHD_INFO(("%s: got a flush signal, but no packets in glom.\n", __FUNCTION__));
		return 0;
	}
...
#if defined(BALONG_PLATFORM)
static int
dhd_bcmsdh_recv_skbbuf(dhd_bus_t *bus, uint32 addr, uint fn, uint flags, uint8 *buf, uint nbytes,
	void *pkt, bcmsdh_cmplt_fn_t complete, void *handle)
{
	int status;
	...
	status = bcmsdh_recv_buf(bus->sdh, addr, fn, flags, buf, nbytes, pkt, complete, handle);
#else
	status = bcmsdh_recv_skbbuf(bus->sdh, addr, fn, flags, buf, nbytes, pkt, complete, handle);
#endif
	return status;
}
...
dhd_glom_dump(void)
{
	printk("[txglom] packet totalnum=%u, txglom num=%u, average-packet-number-txglom=%d\n", g_st_total_packet_txglom, g_st_times_txglom, (int)(g_st_total_packet_txglom/g_st_times_txglom));
	printk("[rxglom] packet totalnum=%u, rxglom num=%u, average-packet-number-rxglom=%d\n", g_st_total_packet_rxglom, g_st_times_rxglom, (int)(g_st_total_packet_rxglom/g_st_times_rxglom));

	g_st_total_packet_txglom = 0;
	g_st_total_packet_rxglom = 0;
	g_st_times_rxglom = 0;
	g_st_times_txglom = 0;
}
#endif
