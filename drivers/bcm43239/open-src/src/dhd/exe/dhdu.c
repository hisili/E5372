 ...
 *
 * $Id: dhdu.c,v 1.88.2.19 2011-01-19 23:47:10 $
 */
...
dhd_download(void *dhd, cmd_t *cmd, char **argv)
{
		...
		if (tmp_len == trx_hdr_len) {
			if (trx_hdr.magic == TRX_MAGIC) {
				...
				if (trx_hdr.flag_version & TRX_OVERLAYS) {
					...
					goto exit;
				} else if (trx_hdr.flag_version & TRX_UNCOMP_IMAGE &&
				!(trx_hdr.flag_version & TRX_ROMSIM_IMAGE)) {

					/* USB/HSIC chips downloads the firmware with trx headers */
					fseek(fp, 0, SEEK_SET);
	...
	if (trx_file) {
		if (bustype == BUS_TYPE_USB)
			fsize = trx_hdr.offsets[0] + trx_hdr.offsets[2] + sizeof(trx_hdr);
		else
			/* For ROMLSIM */
	...
	if (trx_file && bustype == BUS_TYPE_SDIO) {
		/* ROMSIM is currently supported only in SDIO chips */
