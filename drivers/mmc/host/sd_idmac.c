...
void __iomem * balong_internal_base_sd = NULL;

/*global variables*/
static IDMAC_HANDLE_S *pstIdmacHandle_sd = BSP_NULL;
...
static BSP_U32 idmac_tx_qptr_flag_sd[IDMAC_PARTITION_TYPE_MAX][IDMAC_PARTITION_TYPE_MAX]=
 {
    {
...
/*获取数据分段类型*/
__inline__ IDMAC_PARTITION_TYPE_E IDMAC_get_node_partition_sd(BSP_U32 i, BSP_U32 cnt)
{
...
__inline__ BSP_U32 IDMAC_set_qptr_sd(IDMAC_HANDLE_S *imachandle, IDMAC_PARTITION_TYPE_E ePartType,
                IDMAC_PARTITION_TYPE_E ePartType_sp,BSP_U32 buf1, BSP_U32 len1, DmaDesc **desc_over)
{
	...
	desc->status = (next + 1 != imachandle->DescCount)?idmac_tx_qptr_flag_sd[ePartType][ePartType_sp]:(idmac_tx_qptr_flag_sd[ePartType][ePartType_sp]|
		DescEndOfRing);
	desc->buffer1 = buf1;
...
//	idmachandle->RawAddr= dma_alloc_coherent(NULL, sizeof(DmaDesc)*no_of_desc, &dma_addr, GFP_DMA|__GFP_WAIT);
...
static BSP_S32 SD_IDMAC_set_node_ptr(
	BSP_U32 u32buf,
	BSP_U32 u32len,
	BSP_U32 u32NodeCnt,
...
)
{
	...
	if(!pstIdmacHandle_sd)
	{
		hisdio_error("please initialize the dmac first, perform SD_IDMAC_Init");
		return BSP_ERROR;
	}
	...
	for(cnt=0; cnt < u32NodeCnt; cnt++)
	{
		...
		if((0 == cnt) && !addr_align_flag)
		{
			...
			//memcpy((BSP_VOID *)pstIdmacHandle_sd->assisit_buf, (BSP_VOID*)u32buf, IDMAC_get_addr_noalign_bytes(u32buf));
			return BSP_ERROR;
		}

		/*检查数据分段类型*/
		ePartitionType = IDMAC_get_node_partition_sd(cnt, u32NodeCnt);
		...
		switch(ePartitionType)
		{
			...
			case IDMAC_PARTITION_TYPE_HEAD:
				//buf = addr_align_flag?buf_skip_noalign_addr:(BSP_U32)pstIdmacHandle_sd->assisit_buf;
				...
				switch(sg_list_ePartitionType)
				{
					case IDMAC_PARTITION_TYPE_TOTAL:
						...
						pstIdmacHandle_sd->Next = 0;
						buf = u32buf;
						IDMAC_init_desc_base(pstIdmacHandle_sd);
						break;
					case IDMAC_PARTITION_TYPE_HEAD:
						len = IDMAC_BUF_MAXSIZE;
						pstIdmacHandle_sd->Next = 0;
		...
		/*填充描述符*/
		(BSP_VOID)IDMAC_set_qptr_sd(pstIdmacHandle_sd, sg_list_ePartitionType,ePartitionType, buf, len ,&desc);
...
)
{
	...
	for(cnt=0; cnt < u32NodeCnt; cnt++)
	{
		...
		/*填充描述符*/
		(BSP_VOID)IDMAC_set_qptr_sd(pstIdmacHandle_sd, ePartitionType,IDMAC_PARTITION_TYPE_NORMAL,buf, len ,&desc);
...
BSP_VOID SD_IDMAC_NoCopyReceive(	BSP_U32 u32buf, 
									BSP_U32 u32len, 
									BSP_U32 u32NodeCnt)
{
	BSP_U32 cnt = 0;
...
BSP_S32	SD_IDMAC_SingleChannelSendLLI(struct scatterlist *sgHeader, unsigned int dma_len)
{
	unsigned int cnt=0;
	...
	{
		ePartitionType = IDMAC_get_node_partition_sd(cnt, dma_len);
		...
		else if(buf_len > (IDMAC_BUF_MAXSIZE))
		{
			...
			u32NodeCnt = IDMAC_get_node_count(buf_len);
			if(u32NodeCnt < 1)
			{
			...
			if(BSP_OK != SD_IDMAC_set_node_ptr(buf, buf_len, u32NodeCnt, ePartitionType,&descTag))
			{
		...
		/*填充描述符*/
		IDMAC_set_qptr_sd(pstIdmacHandle_sd, ePartitionType,IDMAC_PARTITION_TYPE_NORMAL, buf, len ,&desc);
		u32NodeCnt_new ++;
	...
	if(3 >= HISDIO_TRACE_LEVEL)
	{
	    ...
	    {
	        struct tagDmaDesc* tmp = pstIdmacHandle_sd->Desc + cnt;
...
BSP_S32	 SD_IDMAC_SingleChannelReceiveLLI(struct scatterlist *sgHeader, unsigned int dma_len)
{
	unsigned int cnt=0;
	DmaDesc *desc = BSP_NULL;
	DmaDesc *descTag = BSP_NULL;
	...
	for_each_sg(sgHeader, sgNode, dma_len, cnt) 
	{
		...
		switch(ePartitionType)
		{
			case IDMAC_PARTITION_TYPE_TOTAL:				
				...
				pstIdmacHandle_sd->Next = 0;
				IDMAC_init_desc_base(pstIdmacHandle_sd);
				hisdio_trace(1,"total enter,descdma: 0x%x",(BSP_U32)pstIdmacHandle_sd->DescDma);
				break;
			...
			case IDMAC_PARTITION_TYPE_MID:
				hisdio_trace(1, "next is %d", pstIdmacHandle_sd->Next);
				len = buf_len;
				break;
...
BSP_S32	SD_IDMAC_SingleChannelStart(struct scatterlist *sgHeader, unsigned int dma_len,  enum dma_data_direction direction)
{
	BSP_S32 ret = BSP_OK;
	if(DMA_TO_DEVICE == direction)
	{
		/*memory to peripheral*/
		ret = SD_IDMAC_SingleChannelSendLLI(sgHeader, dma_len);
	}
	else if(DMA_FROM_DEVICE == direction)
	{
		/*peripheral to memory*/
		ret = SD_IDMAC_SingleChannelReceiveLLI(sgHeader, dma_len);
	}
	else
	{
		return BSP_ERROR;
 ...
 *
 *************************************************/
 BSP_S32	SD_IDMAC_Init(void __iomem * base)
 {
	...
	balong_internal_base_sd = base;
	hisdio_trace(1, "internal base : 0x%p", balong_internal_base_sd);
	/*配置内置DMA句柄*/
	pstIdmacHandle_sd = (IDMAC_HANDLE_S*)kmalloc(sizeof(IDMAC_HANDLE_S), GFP_KERNEL);
	/*check ptr alignment, mod should be 0*/
#if 0
	if(!IDMAC_check_addr_align(pstIdmacHandle_sd))
	{
		hisdio_error("pstIdmacHandle_sd not alignment");
		kfree(pstIdmacHandle_sd);
		hisdio_trace(1, "leave");
		return BSP_ERROR;
	}
#endif
	hisdio_trace(1, "pstIdmacHandle_sd addr: 0x%p", pstIdmacHandle_sd);
	memset((BSP_VOID*)pstIdmacHandle_sd, 0x0, sizeof(IDMAC_HANDLE_S));

	/*分配辅助buf:cache line 对齐，linux 没有必要*/
	pstIdmacHandle_sd->assisit_buf = kmalloc(BSP_CACHE_ALIGN_SIZE,GFP_KERNEL);
	if(!pstIdmacHandle_sd->assisit_buf)
	{
		hisdio_error("pstIdmacHandle_sd->assisit_buf alloc failed!");
	}
	hisdio_trace(1, "assisit buf : 0x%p", pstIdmacHandle_sd->assisit_buf);
	//SD_IDMAC_config_init();
	/*创建描述符链*/
	if(BSP_OK != IDMAC_alloc_desc_queue(pstIdmacHandle_sd, IDMAC_DESC_SIZE + 1))
	{
		kfree(pstIdmacHandle_sd->assisit_buf);
		kfree(pstIdmacHandle_sd);
		hisdio_trace(1, "leave");
		return BSP_ERROR;
	}
	...
	pstIdmacHandle_sd->bDescSetupFlag = BSP_TRUE;
	
	hisdio_trace(1, "leave");
 	return BSP_OK;
 }
 
 BSP_VOID SD_IDMAC_Exit(void __iomem * base)
 {
 	/*disable dma control etc.*/
	hisdio_trace(1, "enter");
	if(!pstIdmacHandle_sd)
	{
		hisdio_error("pstIdmacHandle_sd is NULL!");
		return;
	}
	IDMAC_free_desc_queue(pstIdmacHandle_sd);
	kfree(pstIdmacHandle_sd->assisit_buf);
	kfree(pstIdmacHandle_sd);
	/*free memmory*/
	hisdio_trace(1, "leave");
