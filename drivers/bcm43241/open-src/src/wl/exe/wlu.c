 ...
 *
 * $Id: wlu.c 348379 2012-08-01 17:53:09Z $
 */
...
static cmd_func_t wl_assoclist;
...
static void wl_txpwr_array_row_print(int8 *powers, int mimo, int8 channel_bandwidth,
	reg_rate_index_t verbose);
static void wl_txpwr_array_print(int8 *powers, int mimo, int8 channel_bandwidth, bool verbose);
...
cmd_t wl_cmds[] = {
	...
	{ "assoclist", wl_assoclist, WLC_GET_ASSOCLIST, -1,
	...
	"[wsec WEP|TKIP|AES|TKIPAES] [suppress (neither)|found|lost]\n"
	...
	"\t\t       and peer must be TDLS connected\n\n"
	"\t       wl tdls_wfd_ie clr own\n\n"
...
wl_nrate_print(uint32 rspec)
{
	...
	if (encode == WL_RSPEC_ENCODE_RATE) {
		...
		uint Nss = (rspec & WL_RSPEC_VHT_NSS_MASK) >> WL_RSPEC_VHT_NSS_SHIFT;
		printf("vht mcs %d Nss %d Tx Exp %d%s%s%s %s\n",
		       vht, Nss, txexp, stbc, ldpc, sgi, rspec_auto);
	}
}
...
wl_txppr_print(txppr_t *ppr, int cck, int isht)
{
	...
	printf("20MHz:\n");
	if (cck) {
		printf("CCK          %2d %2d %2d %2d\n", ppr->b20_1x1dsss[0], ppr->b20_1x1dsss[1],
		       ppr->b20_1x1dsss[2], ppr->b20_1x1dsss[3]);
		if (isht) {
			printf("CCK CDD 1x2  %2d %2d %2d %2d\n",
			       ppr->b20_1x2dsss[0], ppr->b20_1x2dsss[1],
			       ppr->b20_1x2dsss[2], ppr->b20_1x2dsss[3]);
			printf("CCK CDD 1x3  %2d %2d %2d %2d\n",
			       ppr->b20_1x3dsss[0], ppr->b20_1x3dsss[1],
			       ppr->b20_1x3dsss[2], ppr->b20_1x3dsss[3]);
		}
	}
	printf("OFDM         %2d %2d %2d %2d %2d %2d %2d %2d\n",
	       ppr->b20_1x1ofdm[0], ppr->b20_1x1ofdm[1], ppr->b20_1x1ofdm[2], ppr->b20_1x1ofdm[3],
	       ppr->b20_1x1ofdm[4], ppr->b20_1x1ofdm[5], ppr->b20_1x1ofdm[6], ppr->b20_1x1ofdm[7]);
	printf("OFDM-CDD     %2d %2d %2d %2d %2d %2d %2d %2d\n",
	       ppr->b20_1x2cdd_ofdm[0], ppr->b20_1x2cdd_ofdm[1], ppr->b20_1x2cdd_ofdm[2],
	       ppr->b20_1x2cdd_ofdm[3],
	       ppr->b20_1x2cdd_ofdm[4], ppr->b20_1x2cdd_ofdm[5], ppr->b20_1x2cdd_ofdm[6],
	       ppr->b20_1x2cdd_ofdm[7]);
	for (i = 0; i < n; i++) {
		switch (i + offset) {
			case 0:
				str = "MCS-SISO    ";
			...
			case 1:
				str = "MCS-CDD     ";
				ptr = ppr->b20_1x2cdd_mcs0;
				break;
			case 2:
				str = "MCS STBC    ";
				ptr = ppr->b20_2x2stbc_mcs0;
				break;
			case 3:
				str = "MCS 8~15    ";
				ptr = ppr->b20_2x2sdm_mcs8;
				break;
			case 4:
			case 5:
				ptr = NULL;
			...
			case 10:
				str = "2 Nsts 3 Tx ";
				ptr = ppr->b20_2x3sdm_mcs8;
				break;
			case 11:
				str = "3 Nsts 3 Tx ";
				ptr = ppr->b20_3x3sdm_mcs16;
				break;
			default:
				ptr = NULL;
				break;
	...
	printf("\n40MHz:\n");
	printf("OFDM         %2d %2d %2d %2d %2d %2d %2d %2d\n",
	       ppr->b40_1x1ofdm[0], ppr->b40_1x1ofdm[1], ppr->b40_1x1ofdm[2], ppr->b40_1x1ofdm[3],
	       ppr->b40_1x1ofdm[4], ppr->b40_1x1ofdm[5], ppr->b40_1x1ofdm[6], ppr->b40_1x1ofdm[7]);
	printf("OFDM-CDD     %2d %2d %2d %2d %2d %2d %2d %2d\n",
	       ppr->b40_1x2cdd_ofdm[0], ppr->b40_1x2cdd_ofdm[1], ppr->b40_1x2cdd_ofdm[2],
	       ppr->b40_1x2cdd_ofdm[3],
	       ppr->b40_1x2cdd_ofdm[4], ppr->b40_1x2cdd_ofdm[5], ppr->b40_1x2cdd_ofdm[6],
	       ppr->b40_1x2cdd_ofdm[7]);
	for (i = 0; i < n; i++) {
		switch (i + offset) {
			case 0:
				str = "MCS-SISO    ";
			...
			case 1:
				str = "MCS-CDD     ";
				ptr = ppr->b40_1x2cdd_mcs0;
				break;
			case 2:
				str = "MCS STBC    ";
				ptr = ppr->b40_2x2stbc_mcs0;
				break;
			case 3:
				str = "MCS 8~15    ";
				ptr = ppr->b40_2x2sdm_mcs8;
				break;
			case 4:
			case 5:
				ptr = NULL;
			...
			case 10:
				str = "2 Nsts 3 Tx ";
				ptr = ppr->b40_2x3sdm_mcs8;
				break;
			case 11:
				str = "3 Nsts 3 Tx ";
				ptr = ppr->b40_3x3sdm_mcs16;
				break;
			default:
				ptr = NULL;
				break;
	...
	printf("\n20 in 40MHz:\n");
	if (cck) {
		printf("CCK          %2d %2d %2d %2d\n", ppr->b20in40_1x1dsss[0],
			ppr->b20in40_1x1dsss[1], ppr->b20in40_1x1dsss[2], ppr->b20in40_1x1dsss[3]);
		if (isht) {
			printf("CCK CDD 1x2  %2d %2d %2d %2d\n",
			       ppr->b20in40_1x2dsss[0], ppr->b20in40_1x2dsss[1],
			       ppr->b20in40_1x2dsss[2], ppr->b20in40_1x2dsss[3]);
			printf("CCK CDD 1x3  %2d %2d %2d %2d\n",
			       ppr->b20in40_1x3dsss[0], ppr->b20in40_1x3dsss[1],
			       ppr->b20in40_1x3dsss[2], ppr->b20in40_1x3dsss[3]);
		}
	}
	printf("OFDM         %2d %2d %2d %2d %2d %2d %2d %2d\n",
	       ppr->b20in40_1x1ofdm[0], ppr->b20in40_1x1ofdm[1], ppr->b20in40_1x1ofdm[2],
	       ppr->b20in40_1x1ofdm[3], ppr->b20in40_1x1ofdm[4], ppr->b20in40_1x1ofdm[5],
	       ppr->b20in40_1x1ofdm[6], ppr->b20in40_1x1ofdm[7]);
	printf("OFDM-CDD     %2d %2d %2d %2d %2d %2d %2d %2d\n",
	       ppr->b20in40_1x2cdd_ofdm[0], ppr->b20in40_1x2cdd_ofdm[1],
	       ppr->b20in40_1x2cdd_ofdm[2], ppr->b20in40_1x2cdd_ofdm[3],
	       ppr->b20in40_1x2cdd_ofdm[4], ppr->b20in40_1x2cdd_ofdm[5],
	       ppr->b20in40_1x2cdd_ofdm[6], ppr->b20in40_1x2cdd_ofdm[7]);
	for (i = 0; i < n; i++) {
		switch (i) {
			case 0:
				str = "1 Nsts 1 Tx ";
				ptr = ppr->b20in40_1x1mcs0;
				break;
			case 1:
				str = "1 Nsts 2 Tx ";
				ptr = ppr->b20in40_1x2cdd_mcs0;
				break;
			case 2:
				str = "1 Nsts 3 Tx ";
				ptr = ppr->b20in40_1x3cdd_mcs0;
				break;
			case 3:
				str = "2 Nsts 2 Tx ";
				ptr = ppr->b20in40_2x2sdm_mcs8;
				break;
			case 4:
				str = "2 Nsts 3 Tx ";
				ptr = ppr->b20in40_2x3sdm_mcs8;
				break;
			case 5:
				str = "3 Nsts 3 Tx ";
				ptr = ppr->b20in40_3x3sdm_mcs16;
				break;
			default:
				ptr = NULL;
				break;
 ...
 */
#define CURPOWER_OUTPUT_FORMAT_VERSION "4.4"
...
wl_get_current_power(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* parse */
	if (power.version != TX_POWER_T_VERSION) {
		printf("error: version mismatch - driver %d, wl executable was expecting %d\n",
			power.version, TX_POWER_T_VERSION);
		err = IOCTL_ERROR;
	} else {
		power.flags = dtoh32(power.flags);
		power.chanspec = wl_chspec_from_driver(power.chanspec);
		power.local_chanspec = wl_chspec_from_driver(power.local_chanspec);
		...
		printf("%-23s%s\n", "Channel Width:",
			((power.channel_bandwidth == WL_BW_20MHZ) ? "20MHz" :
			((power.channel_bandwidth == WL_BW_40MHZ) ? "40MHz" : "80MHz")));
		printf("%-23s%d.%d dBm\n", "User Target:",
		...
		wl_rate_print(rate_str, power.last_tx_ratespec);
		printf("[%s] %s\n", get_reg_rate_string_from_ratespec(power.last_tx_ratespec),
			rate_str);
		printf("\n");
		...
		if (brief)
		{
			wl_txpwr_regulatory_array_row_print(power.clm_limits,
				power.clm_limits_subchan1, power.clm_limits_subchan2,
				power.channel_bandwidth,
				get_reg_rate_index_from_ratespec(power.last_tx_ratespec));
		}
		else
		{
			wl_txpwr_regulatory_array_print(power.clm_limits, power.clm_limits_subchan1,
				power.clm_limits_subchan2, power.channel_bandwidth, verbose);
		}
		printf("\n");

		printf("%-23s%d\n", "Core Index:", power.display_core);
		printf("Board Limits:\n");
		if (brief)
		{
			wl_txpwr_array_row_print(power.board_limit, mimo, power.channel_bandwidth,
				get_reg_rate_index_from_ratespec(power.last_tx_ratespec));
		}
		else
		{
			wl_txpwr_array_print(power.board_limit, mimo, power.channel_bandwidth,
				verbose);
			printf("\n");
		}
		...
		if (brief)
		{
			wl_txpwr_array_row_print(power.target, mimo, power.channel_bandwidth,
				get_reg_rate_index_from_ratespec(power.last_tx_ratespec));
		}
		else
		{
			wl_txpwr_array_print(power.target, mimo, power.channel_bandwidth, verbose);
		}
		printf("\n");
		...
		if (mimo) {
			...
			for (i = 0; i < power.rf_cores; i++)
			{
				if (power.target_offsets[i] != WL_RATE_DISABLED)
				{
					power_target = power.tx_power_max[i] -
						power.target_offsets[i];
					printf("%d.%d  ",
...
static void
wl_txpwr_array_row_print(int8 *powers, int mimo, int8 channel_bandwidth,
	reg_rate_index_t rate_index)
{
	...
	const int8 RATE_UNSUPPORTED = -128;
	const char *label;
	int pwr20;
	int pwr20in40;
	int pwr40;
	...
	{
		label = rate_matrix[rate_index].label;

		if (issslpnphy) {
			pwr20 = rate_matrix[rate_index].bw20ssl ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20ssl];
			pwr20in40 = rate_matrix[rate_index].bw20in40ssl ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20in40ssl];
			pwr40 = rate_matrix[rate_index].bw40ssl ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw40ssl];
			pwr80 = rate_matrix[rate_index].bw80ssl ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw80ssl];
			pwr20in80 = rate_matrix[rate_index].bw20in80ssl ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20in80ssl];
			pwr40in80 = rate_matrix[rate_index].bw40in80ssl ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw40in80ssl];
		} else if (isnphy) {
			pwr20 = rate_matrix[rate_index].bw20n ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20n];
			pwr20in40 = rate_matrix[rate_index].bw20in40n ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20in40n];
			pwr40 = rate_matrix[rate_index].bw40n ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw40n];
			pwr80 = rate_matrix[rate_index].bw80n ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw80n];
			pwr20in80 = rate_matrix[rate_index].bw20in80n ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20in80n];
			pwr40in80 = rate_matrix[rate_index].bw40in80n ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw40in80n];
		} else {
			pwr20 = rate_matrix[rate_index].bw20ht ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20ht];
			pwr20in40 = rate_matrix[rate_index].bw20in40ht ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20in40ht];
			pwr40 = rate_matrix[rate_index].bw40ht ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw40ht];
			pwr80 = rate_matrix[rate_index].bw80ht ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw80ht];
			pwr20in80 = rate_matrix[rate_index].bw20in80ht ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw20in80ht];
			pwr40in80 = rate_matrix[rate_index].bw40in80ht ==
				WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
				powers[rate_matrix[rate_index].bw40in80ht];
		}
		...
		case WL_BW_20MHZ:
			...
			pwr80 = RATE_UNSUPPORTED;
			pwr20in80 = RATE_UNSUPPORTED;
			pwr40in80 = RATE_UNSUPPORTED;
			break;
		...
		wl_txpwr_print_row(label, rate_matrix[rate_index].chains,
			pwr20, pwr20in40, pwr40, pwr80, pwr20in80, pwr40in80,
			RATE_UNSUPPORTED, channel_bandwidth, TRUE);
	}
}

static void
wl_txpwr_array_print(int8 *powers, int mimo, int8 channel_bandwidth, bool verbose)
{
	int i, j;
	bool issslpnphy = (mimo == WL_TX_POWER_F_SISO);
	bool isnphy = !issslpnphy && ((mimo & WL_TX_POWER_F_HT) != WL_TX_POWER_F_HT);
	...
	for (i = 0; i < RATE_GROUP_ID_COUNT; i++) {
		...
		case WL_BW_40MHZ:
			if (issslpnphy)
				compress_rate = !verbose &&
					((rate_matrix[first_rate].bw40ssl == WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw40ssl], 0, num_rates)) &&
					((rate_matrix[first_rate].bw20in40ssl ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw20in40ssl], 0, num_rates));
			else if (isnphy)
				compress_rate = !verbose &&
					((rate_matrix[first_rate].bw40n == WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw40n], 0, num_rates)) &&
					((rate_matrix[first_rate].bw20in40n ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw20in40n], 0, num_rates));
			else
				compress_rate = !verbose &&
					((rate_matrix[first_rate].bw40ht == WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw40ht], 0, num_rates)) &&
					((rate_matrix[first_rate].bw20in40ht ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw20in40ht], 0, num_rates));
			break;

		case WL_BW_80MHZ:
			if (issslpnphy)
				compress_rate = !verbose &&
					((rate_matrix[first_rate].bw80ssl == WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw80ssl], 0, num_rates)) &&
					((rate_matrix[first_rate].bw20in80ssl ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw20in80ssl], 0, num_rates)) &&
					((rate_matrix[first_rate].bw40in80ssl ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw40in80ssl], 0, num_rates));

			else if (isnphy)
				compress_rate = !verbose &&
					((rate_matrix[first_rate].bw80n == WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw80n], 0, num_rates)) &&
					((rate_matrix[first_rate].bw20in80n ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw20in80n], 0, num_rates)) &&
					((rate_matrix[first_rate].bw40in80n ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw40in80n], 0, num_rates));
			else
				compress_rate = !verbose &&
					((rate_matrix[first_rate].bw80ht == WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw80ht], 0, num_rates)) &&
					((rate_matrix[first_rate].bw20in80ht ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw20in80ht], 0, num_rates)) &&
					((rate_matrix[first_rate].bw40in80ht ==
					WL_UNSUPPORTED_IDX) ||
					wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].
					bw40in80ht], 0, num_rates));
			break;
		}
		...
		for (; j < num_rates; j++) {
			...
			if (issslpnphy) {
				...
				pwr80 = rate_matrix[first_rate + j].bw80ssl ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw80ssl];
				pwr20in80 = rate_matrix[first_rate + j].bw20in80ssl ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw20in80ssl];
				pwr40in80 = rate_matrix[first_rate + j].bw40in80ssl ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw40in80ssl];
			} else if (isnphy) {
				...
				pwr80 = rate_matrix[first_rate + j].bw80n ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw80n];
				pwr20in80 = rate_matrix[first_rate + j].bw20in80n ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw20in80n];
				pwr40in80 = rate_matrix[first_rate + j].bw40in80n ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw40in80n];
			} else {
				...
				pwr80 = rate_matrix[first_rate + j].bw80ht ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw80ht];
				pwr20in80 = rate_matrix[first_rate + j].bw20in80ht ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw20in80ht];
				pwr40in80 = rate_matrix[first_rate + j].bw40in80ht ==
					WL_UNSUPPORTED_IDX ? RATE_UNSUPPORTED :
					powers[rate_matrix[first_rate + j].bw40in80ht];
			}
			...
			wl_txpwr_print_row(label, rate_matrix[first_rate + j].chains,
				pwr20, pwr20in40, pwr40, pwr80, pwr20in80, pwr40in80,
				RATE_UNSUPPORTED, channel_bandwidth, verbose);
		}
	}
}
	...
	int8 *powers_subchan2, int8 channel_bandwidth,  reg_rate_index_t rate_index)
{
	...
	{
		label = rate_matrix[rate_index].label;
		chains = rate_matrix[rate_index].chains;

		switch (channel_bandwidth) {
		case WL_BW_20MHZ:
			pwr20 = powers[rate_matrix[rate_index].rate];
			break;
		case WL_BW_40MHZ:
			pwr40 = powers[rate_matrix[rate_index].rate];
			pwr20in40 = powers_subchan1[rate_matrix[rate_index].rate];
			break;
		case WL_BW_80MHZ:
			pwr80 = powers[rate_matrix[rate_index].rate];
			pwr40in80 = powers_subchan1[rate_matrix[rate_index].rate];
			pwr20in80 = powers_subchan2[rate_matrix[rate_index].rate];
			break;
		}
	...
	int8 *powers_subchan2, int8 channel_bandwidth, bool verbose)
{
	...
	for (i = 0; i < RATE_GROUP_ID_COUNT; i++) {
		...
		case WL_BW_20MHZ:
			compress_rate = !verbose &&
				wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].rate], 0,
				num_rates);
			break;
		case WL_BW_40MHZ:
			compress_rate = !verbose &&
				wl_array_uniform((uint8*)&powers[rate_matrix[first_rate].rate], 0,
				num_rates) &&
				wl_array_uniform((uint8*)&powers_subchan1[rate_matrix
		...
		case WL_BW_80MHZ:
			compress_rate = !verbose &&
				...
				wl_array_uniform((uint8*)&powers_subchan1[rate_matrix
				[first_rate].rate], 0, num_rates) &&
				wl_array_uniform((uint8*)&powers_subchan2[rate_matrix
				[first_rate].rate], 0, num_rates);
			break;
		}
		...
		for (; j < num_rates; j++) {
			...
			case WL_BW_40MHZ:
				...
				pwr20in40 = powers_subchan1[rate_matrix[first_rate + j].rate];
				break;
			case WL_BW_80MHZ:
				pwr80 = powers[rate_matrix[first_rate + j].rate];
				pwr40in80 = powers_subchan1[rate_matrix[first_rate + j].rate];
				pwr20in80 = powers_subchan2[rate_matrix[first_rate + j].rate];
				break;
			}
			wl_txpwr_print_row(label, chains, pwr20, pwr20in40,
				pwr40, pwr80, pwr20in80, pwr40in80, RATE_UNSUPPORTED,
...
#define WL_MAX_STA_NUM 12
static int
wl_assoclist(void *wl, cmd_t *cmd, char **argv)
{
	int ret;
	struct maclist *maclist = (struct maclist *) buf;
	struct ether_addr *ea;
	uint i, max = WL_MAX_STA_NUM ;
	uint len=max*ETHER_ADDR_LEN + sizeof(int);
	...
	if (!*++argv) {
		...
		if ((ret = wlu_get(wl, cmd->get, maclist, len)) < 0)
			return ret;
...
wl_tdls_wfd_ie(void *wl, cmd_t *cmd, char **argv)
{
	...
	int ret;
	uint8* ptr;
	uint8 length, element, offset;
	uint8 buffer[TDLS_WFD_IE_SIZE - (WFA_OUI_LEN + 3)];
	uint16 element_length, current_length;
	bcm_tlv_t * ie;
	unsigned long value;
	struct ether_addr ea;
	...
	if ((ie = bcm_parse_tlvs(ptr + offset, length - offset, DOT11_MNG_VS_ID)) != NULL) {
		if (ie->len > WFA_OUI_LEN + 1) {
			if ((!memcmp(ie->data, WFA_OUI, WFA_OUI_LEN)) &&
				(*(ie->data + WFA_OUI_LEN) == WFA_OUI_TYPE_WFD)) {
					/* WFD */
					memcpy(buffer + current_length, ie->data + WFA_OUI_LEN + 1,
						ie->len - WFA_OUI_LEN - 1);
					current_length += ie->len - WFA_OUI_LEN - 1;
			}
		}
		offset = (uint8)((uint8*)ie - ptr + ie->len + 2);
	}
	else break;
			}
	...
	printf("%u\n", (ptr[5] << 8) + ptr[6]);
		...
		/* IP specified */
			...
			if (info.length != 32) {
				...
				"\t8-byte subelement 0 (WFD device information)\n"
				"\t8-byte subelement 1 (BSSID)\n"
				"\t7-byte subelement 8 (IP address)\n");
				return USAGE_ERROR;
			}
...
#if defined (WIFI_AT_INTERFACE)
    /*get the rx packets information*/
    sprintf(wifi_at_cmdbuf, WL_RX_REPORT,dtoh32(cnt->rxframe),dtoh32(cnt->rxerror));

    if(g_iAtDevFd)
    {
        write(g_iAtDevFd,wifi_at_cmdbuf,strlen(wifi_at_cmdbuf) + 1);
    }
#endif
...
#if defined (WIFI_AT_INTERFACE)
            /*get the rx packets information*/
            sprintf(wifi_at_cmdbuf, WL_RX_PACKET, dtoh32(cnt->pktengrxducast),
                    dtoh32(cnt->pktengrxdmcast));
	...
	while (*++argv) {
		if (!stricmp(*argv, "bssid")) {
			if (*++argv) {
				...
				if (!wl_ether_atoe(*argv, &pbssid->macaddr))
				{
...
wl_pkteng(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (strcmp(cmd->name, "pkteng_stop") == 0) {
		...
		if ((strcmp(*argv, "tx") == 0) || (strcmp(*argv, "txwithack") == 0))  {
            ...
            if (g_iAtDevFd)
            {
                write(g_iAtDevFd,WL_TX_CMD,sizeof(WL_TX_CMD));
            }
#endif
            ...
            if (g_iAtDevFd)
            {
                write(g_iAtDevFd,WL_RX_CMD,sizeof(WL_RX_CMD));
            }
#endif
			if ((strcmp(*argv, "rx") == 0))
...
wl_rxiq(void *wl, cmd_t *cmd, char **argv)
{
	 ...
	 */
	rxiq = (extra_gain_3dBsteps << 28) | (gain_correct << 24) | (lpf_hpc << 20) |
...
wl_wowl_extended_magic(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((ret = wlu_get(wl, cmd->get, arg, WLC_IOCTL_MAXLEN)))
		return ret;
