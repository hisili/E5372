...
static struct amba_device uart0_device =
{
	...
	.res =
	{
		.start	= PV500V1_UART2_BASE,
		.end	= PV500V1_UART2_BASE + SZ_4K - 1,
		.flags	= IORESOURCE_MEM,
	},
	.dma_mask	= ~0,
	.periphid   = 0x000c21c0,
	.irq		= { INT_LVL_UART_2, NO_IRQ },
};
...
static struct map_desc pv500v1_io_desc[] __initdata = {
	...
	/*sys control reg*/
	{
		...
		.length		= INTEGRATOR_SC_SIZE,
		.type		= MT_DEVICE,
	},
	/*uart2 control reg*/
    {
		.virtual	= IO_ADDRESS(PV500V1_UART2_BASE),
		.pfn		= __phys_to_pfn(PV500V1_UART2_BASE),
    ...
    {
            .virtual        = IO_ADDRESS(0x50120000),
            .pfn            = __phys_to_pfn(0x50120000),
            .length = 0x10000,
            .type   = MT_DEVICE,
    },
    /* om board type */
    {
            .virtual        = IO_ADDRESS(BOARD_TYPE_REG_ADDR),
            .pfn            = __phys_to_pfn(BOARD_TYPE_REG_ADDR),
            .length = 0x10000,
            .type   = MT_DEVICE,
    },
    /*SOCP,4k*/
    {   
        .virtual   = IO_ADDRESS(SOCP_REG_BASEADDR),
        .pfn       = __phys_to_pfn(SOCP_REG_BASEADDR),
        .length     = SOCP_REG_SIZE,
        .type     = MT_DEVICE,
    },
    {   
	...
	/*NV backup,192K*/
    {		
    	.virtual	= ECS_NV_BASE - ECS_MDDR_RESERVE_ADDR + 0xF3400000, /*(0xF3400000 + 0x01FB0000)*/		
    	.pfn		= __phys_to_pfn(ECS_NV_BASE),		
    	.length		= ECS_NV_SIZE,		
    ...
    /*TTF memory,20MB*/
    {
        .virtual    = ECS_TTF_MEM_BASE - ECS_MDDR_RESERVE_ADDR + 0xF3400000,
        .pfn        = __phys_to_pfn(ECS_TTF_MEM_BASE),
        .length     = ECS_TTF_MEM_SIZE,
        .type      = MT_DEVICE,
    },

    /*DICC,128KB*/
    {
        .virtual    = ECS_TTF_DICC_BASE - ECS_MDDR_RESERVE_ADDR + 0xF3400000,
        .pfn        = __phys_to_pfn(ECS_TTF_DICC_BASE),
...
#endif
	uart0_device.irq[0]	= INT_LVL_UART_2;
