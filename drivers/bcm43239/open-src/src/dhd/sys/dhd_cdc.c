...
dhdcdc_set_ioctl(dhd_pub_t *dhd, int ifidx, uint cmd, void *buf, uint len, uint8 action)
{
	...
	DHD_TRACE(("%s: Enter\n", __FUNCTION__));
	DHD_CTL(("%s: cmd %d len %d\n", __FUNCTION__, cmd, len));
    if(  WLC_GET_VAR == cmd || WLC_SET_VAR ==cmd){
        DHD_CTL(("%s: The cmd is %s\n", __FUNCTION__,buf));    
    }
...
//add by anthony to improve linux server throughput
#ifdef RAISE_BK_PRIO
	if ( h->priority < 3 ) {
		h->priority = 3;
	}
	...
	h->dataOffset = 0;
#if defined(BCMDHDUSB)
	dbus_get_revinfo(dhd->dbus, &chipnum, &chiprev);
	if (chipnum == 0x4330 && chiprev < 3) {
		len = PKTLEN(dhd->osh, pktbuf);
		war_len = ROUNDUP(len, 64);
		BDC_SET_PAD_LEN(h, war_len - len);
		PKTSETLEN(dhd->osh, pktbuf, war_len);
	}
	
	if ( chipnum == BCM43239_CHIP_ID ){
		DHD_DATA((" Padlength for %p is %d  \n",PKTDATA(dhd->osh, pktbuf),pad_len));
		BDC_SET_PAD_LEN(h,pad_len);
	}
...
#if 0
	{
		...
		uchar *info = (uchar *)PKTDATA(dhd->osh, pktbuf);
		int j = 0;
		
		WIFI_TRACE(("%s: 200: tx skb len: %d,buf:%p\n",__FUNCTION__, PKTLEN(dhd->osh,pktbuf),PKTDATA(dhd->osh, pktbuf)));
		
		WIFI_TRACE(("%s: 280: tx skb content: \n",__FUNCTION__));
		for(j = 0; j < 32; j++)
		{
...
dhd_prot_init(dhd_pub_t *dhd)
{
	...
	//ret = dhd_custom_get_mac_address(e5_nv_mac);
	...
	{
		int i = 0;
		DHD_TRACE(("%s: 200: mac to set: ", __FUNCTION__));
		for(i = 0; i < 6; i++)
		{
			DHD_TRACE(("%02x:", e5_nv_mac[i]));
	...
	DHD_TRACE(("%s: 320: finishe dbus_set_revinfo \n", __FUNCTION__));
