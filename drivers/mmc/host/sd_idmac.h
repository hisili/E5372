...
extern void __iomem *balong_internal_base_sd;
...
#else
#define IDMAC_get_addr_align_bytes(buf,len)\
        ((len>IDMAC_get_addr_noalign_bytes(buf))?(len-IDMAC_get_addr_noalign_bytes(buf)):0)
#endif
...
#else
#define IDMAC_get_node_count(len)\
        (((len)+(IDMAC_BUF_MAXSIZE-1))/IDMAC_BUF_MAXSIZE)
#endif
...
#define IDMAC_get_desc_count(buf,len)\
    (IDMAC_check_addr_align((buf))\
        ?IDMAC_get_node_count((len))\
        :(IDMAC_get_node_count(IDMAC_get_addr_align_bytes((buf),(len)))+1))
#endif
...
/*寄存器基址配置*/
#define SD_REGBASE_ADDR		balong_internal_base_sd//0x900AD000~0x900ADFFF
...
BSP_VOID  SD_IDMAC_config_init(void);
BSP_VOID SD_IDMAC_NoCopySend(BSP_U32 u32buf, BSP_U32 u32len, BSP_U32 s32NodeCnt);
BSP_VOID SD_IDMAC_NoCopyReceive(BSP_U32 u32buf, BSP_U32 u32len, BSP_U32 s32NodeCnt);
